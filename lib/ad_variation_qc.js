...
Also write a TITLE: a UNIQUE, punchy 4-6 word label for THIS specific ad.
CRITICAL — IGNORE the banner/caption text overlaid on the video. That banner is IDENTICAL on every variation, so it can NEVER be the title. Do NOT read the on-screen text.
Watch and listen to the WHOLE ad end to end — the spoken hook, opener, body, AND call-to-action — not just the opening few seconds. Base the title on the specific argument this ad makes and what it emphasizes overall.
Make it DISTINCT from sibling variations: several ads share the same opening hook, so do NOT title by the hook alone — find what is unique to THIS ad (its body example, its specific framing or proof, its CTA emphasis) and title from that, so no two ads end up with the same label.
Title Case, no surrounding quotes, no trailing period. Good examples (note how each is specific and different from the others): AP Transfer Credits Angle · Save Time With AP Credits · AP Credits & College Cost · Graduation Planning With AP · College Pathway Via AP Credits · AP Course Strategy Angle. NEVER output the banner line (e.g. "5s Are Made In Summer") or generic labels like "AP Ad", "Variation".
