...
const DEFAULT_FONT = 'Montserrat Bold';
...
export async function overlayBanner({ inputPath, outputPath, bannerConfig, timeoutMs = 600_000 }) {
  ...
  const font = bannerConfig.font || DEFAULT_FONT;
  ...
  const mainText = escText(bannerConfig.text);
  const subtext = bannerConfig.subtext ? escText(bannerConfig.subtext) : null;
  ...
  filter += `,drawtext=text='${mainText}':fontfile=:fontsize=${fontSize}:fontcolor=0x${textColor}:font='${font}':x=(w-text_w)/2:y=${mainY}`;

  if (subtext) {
    filter += `,drawtext=text='${subtext}':fontfile=:fontsize=${subtextFontSize}:fontcolor=0x${textColor}:font='${font}':x=(w-text_w)/2:y=${subtextY}`;
  }
