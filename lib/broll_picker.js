 ...
 */
export function buildPrompts({ transcript, library, totalDuration, brollDensity }) {
...
- SOURCE PREFERENCE — STRONGLY prefer client-provided assets (provenance="client") over Pixabay stock (provenance="pixabay") when relevance is comparable. Pixabay is a SUPPLEMENTAL fallback meant to fill gaps the client library cannot cover; it should NEVER take over the edit when usable client assets exist. If a client asset and a Pixabay asset are roughly equally relevant for a moment, ALWAYS pick the client asset. Only pick a Pixabay asset when no client asset reasonably fits the spoken moment.
