...
/**
 * Download every broll referenced by the insertions list. Resolves the
 * download URL with `file_url ?? storage_url` — `file_url` (legacy/manual
 * upload column) is preferred when present, but the synced-asset flow
 * populates `storage_url` instead, so both are accepted. Errors out clearly
 * if a row has neither (e.g. drive_file_id-only rows — M2 doesn't resolve
 * Drive OAuth).
 *
