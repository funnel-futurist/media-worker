...
import { resolveAiEditMode } from './ai_edit_mode.js';
...
export function buildPipelineOpts(req) {
  return {
    ...
    // 2026-06-01: aiEditMode preset (client-facing edit style). Route validates
    // the enum; we pass the raw value through so resolveAiEditMode (called at
    // the top of runCleanModePipeline) can compute the final skipBroll +
    // introHookEnabled with explicit-flag-wins precedence. Undefined here means
    // "no preset" — resolver falls back to DEFAULT_AI_EDIT_MODE.
    aiEditMode: req.options?.aiEditMode,
...
export async function runCleanModePipeline(req) {
  ...
  // 2026-06-01: aiEditMode preset resolution. Bundles cleanup + captions +
  // AI hook title + (optionally) b-roll into a single client-facing edit-style
  // choice. The resolver is a DEFAULT-SETTER — explicit `skipBroll` /
  // `introHookEnabled` on req.options still win (backward-compat). Mutates
  // opts so the rest of the pipeline reads the resolved values through opts
  // exactly as it always has. See lib/ai_edit_mode.js.
  // Read explicit-flag presence from req.options directly (NOT opts) so the
  // existing `opts.introHookEnabled = req.options?.introHookEnabled === true`
  // and `opts.skipBroll = req.options?.skipBroll === true` coercions in
  // buildPipelineOpts don't mask "caller explicitly set to false".
  const aiEditModeResolved = resolveAiEditMode({
    aiEditMode: opts.aiEditMode,
    explicit: {
      skipBroll: req.options?.skipBroll,
      introHookEnabled: req.options?.introHookEnabled,
    },
  });
  opts.aiEditMode = aiEditModeResolved.aiEditMode;
  opts.skipBroll = aiEditModeResolved.skipBroll;
  opts.introHookEnabled = aiEditModeResolved.introHookEnabled;
  console.log(
    `[ai-edit-mode:${jobId}] aiEditMode=${aiEditModeResolved.aiEditMode} ` +
    `brollEnabled=${!aiEditModeResolved.skipBroll} ` +
    `introHookEnabled=${aiEditModeResolved.introHookEnabled} ` +
    `source=${aiEditModeResolved.source}`,
  );
  ...
  try {
    ...
    // 2026-06-01: surface spoken intro/date/title slate removal in a
    // standalone, easy-to-grep log line. The detector itself is the
    // existing semantic slate_detect; this line just makes the run
    // observable from Railway without parsing the wider step log.
    if (slateMetadata) {
      console.log(
        `[spoken-intro-slate:${jobId}] detected=true ` +
        `start=0.00 end=${slateMetadata.end.toFixed(2)} ` +
        `reason=${slateMetadata.identifier ?? 'spoken_intro_slate'}`,
      );
    } else {
      console.log(`[spoken-intro-slate:${jobId}] detected=false`);
    }
    ...
    if (opts.rawVideoCleanup) {
      ...
      try {
        ...
        cleanupRes = await runRawVideoCleanup({
          ...
          segmentHint: opts.segmentHint,            // null in 1A; 1B will populate
    ...
    return {
      ...
      aiEditMode: opts.aiEditMode,                         // 2026-06-01: surface the resolved preset
    ...
    return {
      ...
      aiEditMode: opts?.aiEditMode ?? null,                // 2026-06-01: include even on partial failures
      inputValidation,
