  ...
  try {
    ...
    if (introHook.enabled) {
      ...
      if (!hookResult.ok) {
        ...
        warnings.push(`intro hook skipped: ${hookResult.reason}${hookResult.detail ? ` — ${String(hookResult.detail).slice(0, 160)}` : ''}`);
      } else {
        ...
        try {
          ...
          warnings.push(`intro card render skipped: ${err instanceof Error ? err.message : String(err)}`.slice(0, 240));
        }
        ...
        if (renderOk) {
          ...
          try {
            ...
            warnings.push(`intro concat skipped: ${err instanceof Error ? err.message : String(err)}`.slice(0, 240));
          }
