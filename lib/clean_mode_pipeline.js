...
import { runRawVideoCleanup } from './raw_video_cleanup.js';
import { analyzeBestTakeAndCta } from './best_take_cta.js';
import { detectSegments } from './detect_segments.js';
import { detectFaceOffsetX, buildCropXExpression, buildCropYExpression } from './face_detect.js';
import { searchPixabayVideos, downloadPixabayVideo } from './pixabay_video.js';
import { searchPexelsVideos, downloadPexelsVideo } from './pexels_video.js';
import { generateStockKeywords } from './stock_keyword_gen.js';
import { shouldFetchStock, mergeStockIntoLibrary, rebalanceClientFirst, isGenericSceneryHit } from './stock_library_merge.js';
import { formatInsertionDiagnostics } from './broll_diagnostics.js';
...
export function buildPipelineOpts(req) {
  return {
    ...
    // Tier 1 follow-up (PR-AP-fix): per-run b-roll exclude list. The route
    // validates it and the candidate-pool filter reads opts.brollExcludeAssetIds
    // (clean_mode_pipeline.js ~L1141), but the Tier 1 merge omitted this
    // req.options → opts write — so every exclude silently no-op'd (Chelsea's
    // Thursday excludes leaked: 1c2817be + fc521f7e appeared despite being listed).
    // This is the documented buildPipelineOpts passthrough-gap class.
    brollExcludeAssetIds: req.options?.brollExcludeAssetIds,
    skipSubtitles: req.options?.skipSubtitles === true,
    // Skip slate detection entirely — for when title/hook share key tokens
    // causing false-positive cuts. Operator must pre-trim the spoken intro.
    skipSlate: req.options?.skipSlate === true,
    // 2026-06-10: per-job cut-precision tunables. detectAndClassifyCuts
    // already accepted these — the pipeline used to hardcode them so per-
    // client editing_defaults overrides silently no-op'd. Surfaced after
    // Chelsea flagged Mon 0:50 phrase cut, Sun 0:43 'protecting' clip, and
    // Fri first-half pauses still present in the Jun 8-14 batch.
    //   cutSafetyMode: 'safe_only' | 'safe_and_soft' | 'all' (default 'safe_only')
    //     - 'safe_only' → only post-sentence / filler / repeat cuts fire (conservative)
    //     - 'safe_and_soft' → also fire phrase-boundary-comma cuts (removes more pauses)
    //   retainSec: number in (0, 1] (default 0.15)
    //     - Pad on each side of a silence cut. Bigger value = more breathing
    //       room before/after surrounding words, less risk of nipping a word's
    //       consonant/breath onset (ffmpeg silencedetect's amplitude-based span
    //       end can extend ~50-100ms past the word boundary Deepgram tags).
    cutSafetyMode: typeof req.options?.cutSafetyMode === 'string'
      ? req.options.cutSafetyMode
      : undefined,
    retainSec: typeof req.options?.retainSec === 'number'
      ? req.options.retainSec
      : undefined,
    // 2026-06-10: preserve brand all-caps stylization in the AI hook title
    // overlay (and other downstream text). Whitelist of terms (e.g.
    // ['ENABLE']) to force ALL-CAPS post-Gemini, since the hook prompt's
    // "Title Case" rule otherwise produces "Enable". Driven by per-client
    // editing_defaults; the portal passes whatever the client row has.
    brandTermsAllCaps: Array.isArray(req.options?.brandTermsAllCaps)
      ? req.options.brandTermsAllCaps
      : undefined,
    // 2026-06-10: deterministic hook fallback derived from the content
    // item title when LLM (Pro + Flash) hook generation fails. The portal
    // passes `content_items.title` here so reels never ship without an
    // overlay just because the LLM emitted a non-validating hook. Same
    // source the portal uses for slateHint extraction (PR #354).
    titleForHookFallback: typeof req.options?.titleForHookFallback === 'string'
      ? req.options.titleForHookFallback
      : undefined,
    ...
    // Tier 2-a (2026-05-27): Pexels as second stock provider.
    // Default: enabled IF PEXELS_API_KEY is set on the worker; orchestrator
    // skips Pexels gracefully when the key is missing. Per-job
    // `pexelsEnabled: false` explicitly disables. Backward-compatible:
    // existing jobs without PEXELS_API_KEY behave identically to pre-Tier-2.
    pexelsEnabled: req.options?.pexelsEnabled !== false,
    pexelsMaxClips: typeof req.options?.pexelsMaxClips === 'number'
      ? req.options.pexelsMaxClips
      : Number(process.env.PEXELS_VIDEO_MAX_CLIPS ?? 6),
    ...
    // PR-AO: optional slate hint. When provided, the slate detector fuzzy-
    // matches against early transcript sentences and cuts through any match
    // even if it sits past the last meta marker. Used for Phil's new
    // recording pattern (date + title only, no trailing "Selected option").
    // Default undefined — no behavior change for callers that don't pass it.
    slateHint: req.options?.slateHint,
    ...
    deepgramKeywords: Array.isArray(req.options?.deepgramKeywords)
      ...
      : undefined,
    // Phase 1A: raw_video_cleanup pre-edit stage. Opt-in (default OFF). When
    // on, a Gemini transcript pass removes repeated takes / restarts / setup
    // chatter / dead air / failed attempts BEFORE cutApply, so cut.mp4 becomes
    // the cleaned base. cleanupMaxFraction is a SAFETY ceiling (not a target);
    // cleanupMinRemainingSec is a hard floor on remaining duration. undefined
    // → raw_video_cleanup applies its own conservative defaults (0.85 / 8s).
    rawVideoCleanup: req.options?.rawVideoCleanup === true,
    // Phase 1B: OPT-IN segment detection + logging (telemetry on how often
    // multi-part uploads happen + feeds a boundary-aware segmentHint into
    // cleanup). Default OFF. 2026-06-09: NO LONGER auto-runs with rawVideoCleanup
    // — multi-part is deprioritized (operators pre-segment + upload single
    // clips), so it only runs when explicitly requested here.
    segmentDetect: req.options?.segmentDetect === true,
    cleanupMaxFraction: typeof req.options?.cleanupMaxFraction === 'number'
      ? req.options.cleanupMaxFraction
      : undefined,
    cleanupMinRemainingSec: typeof req.options?.cleanupMinRemainingSec === 'number'
      ? req.options.cleanupMinRemainingSec
      : undefined,
    // Phase 1B (forward-compat): detected segment boundaries. Null in 1A; when
    // present, raw_video_cleanup clamps every cut to within a single segment so
    // a cut can never span a boundary (never joins Ad 1's end to Ad 2's middle).
    segmentHint: req.options?.segmentHint ?? null,
    // Phase 2: best-take + end-on-CTA (transcript-only). Both default OFF.
    // endOnCta — AUTO-TRIM trailing content after the final CTA (heavily gated).
    // bestTakeDetect — DETECT + LOG a best-take keep/drop proposal (never cuts
    // in v1). Either flag runs the analysis; only endOnCta can apply a trim.
    // ctaTrailBufferSec — seconds kept after the CTA so it isn't clipped.
    endOnCta: req.options?.endOnCta === true,
    bestTakeDetect: req.options?.bestTakeDetect === true,
    ctaTrailBufferSec: typeof req.options?.ctaTrailBufferSec === 'number'
      ? req.options.ctaTrailBufferSec
      : undefined,
  };
}
...
export async function runCleanModePipeline(req) {
  ...
  // PR-AP: cut.mp4 + insertions.json + recompose-manifest.json for the
  // /recompose-broll endpoint. Persisted alongside the final video so a
  // future surgical b-roll swap can re-run compose without re-cutting or
  // re-picking. Populated incrementally as the pipeline steps complete.
  let recomposeAssets = null;
  const recomposeManifest = {
    schemaVersion: 1,
    jobId,
    cutDurationSec: null,
    insertions: [],
    composeOptions: {
      ...
      faceCropOffsetX: null,
      faceCropOffsetY: null,
    },
    introHook: { applied: false },
    banner: { enabled: opts.bannerEnabled === true, config: opts.bannerConfig ?? null },
    bgm: { enabled: false },
    captionStyle: opts.captionStyle ?? 'reel',
    deepgramKeywords: opts.deepgramKeywords ?? null,
  };
  ...
  try {
    ...
    if (word_timestamps.length === 0) {
      ...
      // Terminal failure: re-running won't change the result for the same
      // source. The container/codec is fine (silenceDetect already decoded
      // the audio above) — the audio simply has no recognizable speech
      // (silent, music-only, or the wrong asset was attached). Lead with a
      // human-readable cause; keep the "Deepgram returned 0 words" phrase so
      // inferStepFromErrorMessage still tags this step:'transcribe' and
      // isNoSpeechError() flags it terminal (skips the hourly retry sweep).
      throw new Error(
    ...
    let slateError = null;
    // When title/hook share tokens the LLM false-positives on the hook; skip entirely.
    if (opts.skipSlate === true) {
      console.log(`[clean_mode_pipeline:${jobId}] slate detection SKIPPED via opts.skipSlate=true`);
      steps.slateDetect = {
        ms: Date.now() - stepT,
        detected: false,
        via: 'skipped',
        skipReason: 'opts.skipSlate=true',
      };
    } else {
    try {
      slateMetadata = await detectSlate({
        ...
        // PR-AO: optional slate hint for fuzzy title matching. Phil's new
        // recording style drops the trailing "Selected option / Final
        // revised version" markers, so the title sits past the date and
        // the meta-marker look-ahead can't reach it. When the caller knows
        // the title (filename / content_items.title), we pass it here so
        // the deterministic floor can cut through it explicitly. Default
        // behavior (no hint) is unchanged.
    ...
    cutResult = detectAndClassifyCuts(word_timestamps, {
      ...
      // 2026-06-10: per-job override. Default 'safe_only' kept for back-compat
      // with the talking_head_reel baseline. Pass 'safe_and_soft' to also fire
      // 'soft' deadAir cuts (phrase_boundary_comma) so more mid-sentence
      // pauses get removed for clients where the safety classifier is too
      // conservative (EnableSNP Fri 2026-06-13 left ~5 audible pauses in
      // the first half because they were tagged 'soft').
      cutSafetyMode: opts.cutSafetyMode ?? 'safe_only',
      preserveEmphasisPauses: true,
      // 2026-06-10: per-job override of the start/end retain pad around each
      // silence cut. Default 0.15s. Bump (e.g. 0.25s) when ffmpeg silencedetect's
      // amplitude-based span end extends slightly into the next word's
      // consonant/breath onset — that nipped the first ~80ms of words like
      // "uneasy" / "protecting" on EnableSNP Mon Jun 8 + Sun Jun 14.
    ...
    // ── 4b-1. Segment detection (Phase 1B) — detect + LOG only, no split ──
    // Runs on the FULL RAW transcript BEFORE any cleanup cuts so boundary
    // phrases ("next one", a new intro) are still present. Phase 1B never
    // splits the upload (that's 1C + a child-content_items migration); it only
    // logs the boundaries and feeds them to raw_video_cleanup as a segmentHint
    // so cleanup never cuts ACROSS a detected reset. OPT-IN ONLY (`segmentDetect`).
    // 2026-06-09: decoupled from `rawVideoCleanup` — multi-part is deprioritized
    // (operators pre-segment and upload single clips), so auto-running detection
    // on every cleanup job was a wasted Gemini pass that always returned "single
    // clip". An explicit `segmentHint` in the request still feeds cleanup without
    // running detection. Non-fatal: on error we treat the upload as a single clip.
    let effectiveSegmentHint = opts.segmentHint ?? null; // explicit override wins
    if (opts.segmentDetect) {
      stepT = stepStart('segmentDetect');
      let segmentResult = null;
      let segmentError = null;
      try {
        segmentResult = await detectSegments({
          transcript,
          wordTimestamps: word_timestamps,
          sourceDuration,
          silenceMap: mergedSilenceMap,
        });
      } catch (err) {
        segmentError = err.message;
        console.warn(
          `[clean_mode_pipeline:${jobId}] segment_detect failed (${err.message}) — ` +
          `treating upload as a single clip`,
        );
      }
      const segClips = segmentResult?.clips ?? [];
      const multiClip = segmentResult?.detectedMultipleClips === true && segClips.length > 1;
      // Only constrain cleanup when MULTIPLE clips are detected. A single clip
      // means "no boundary" → leave segmentHint null (Phase 1A behavior).
      if (!effectiveSegmentHint && multiClip) {
        effectiveSegmentHint = { clips: segClips };
      }
      steps.segmentDetect = {
        ms: Date.now() - stepT,
        detectedMultipleClips: multiClip,
        clips: multiClip ? segClips.length : 1,
        boundaries: segClips.map((c) => [Number(c.start.toFixed(2)), Number(c.end.toFixed(2))]),
        ...(segmentError ? { error: segmentError } : {}),
      };
      if (multiClip) {
        console.log(
          `[clean_mode_pipeline:${jobId}] segment_detect: ${segClips.length} clips detected — ` +
          `boundaries logged, cleanup will stay within them (NO split in Phase 1B)`,
        );
        for (const c of segClips) {
          console.log(
            `[clean_mode_pipeline:${jobId}] [segment] clip ${c.clipIndex} ` +
            `[${c.start.toFixed(2)}, ${c.end.toFixed(2)}] "${c.titleGuess ?? ''}" — ${c.reason ?? ''}`,
          );
        }
      } else {
        console.log(`[clean_mode_pipeline:${jobId}] segment_detect: single clip (no multi-part split)`);
      }
    }

    // ── 4b-2. Raw video cleanup (transcript-only, Phase 1A) — opt-in ────
    // Removes repeated takes / restarts / "let me try that again" / setup
    // chatter / dead air / failed attempts that silence + bad-take detection
    // miss. Pushes its removeCuts into cutResult.applied (same pattern as the
    // bad-take block above) so the existing appliedCuts derivation + silence
    // audit + cutApply pick them up — NO second render flow. Conservative +
    // budget-capped (ceiling fraction + min-remaining floor). Failure is
    // NON-FATAL: on error we keep the existing cuts and ship.
    if (opts.rawVideoCleanup) {
      stepT = stepStart('rawVideoCleanup');
      let cleanupRes = null;
      let cleanupError = null;
      try {
        const existingCleanupCuts = cutResult.applied.map((c) => ({ start: c.start, end: c.end }));
        cleanupRes = await runRawVideoCleanup({
          transcript,
          wordTimestamps: word_timestamps,
          sourceDuration,
          silenceMap: mergedSilenceMap,
          existingCuts: existingCleanupCuts,
          segmentHint: effectiveSegmentHint,        // Phase 1B: detected boundaries (null when single clip)
          startAfterSec: slateMetadata?.end ?? 0,
          maxCleanupFraction: opts.cleanupMaxFraction,
          minRemainingSec: opts.cleanupMinRemainingSec,
        });
      } catch (err) {
        cleanupError = err.message;
        console.warn(
          `[clean_mode_pipeline:${jobId}] raw_video_cleanup failed (${err.message}) — ` +
          `continuing without cleanup cuts`,
        );
      }
      const cleanupCuts = cleanupRes?.removeCuts ?? [];
      for (const rc of cleanupCuts) {
        cutResult.applied.push({
          start: rc.start,
          end: rc.end,
          category: 'raw_cleanup',
          reason: `raw_cleanup: ${rc.reason}`.slice(0, 80),
          safety: 'safe',
          safetyReason: 'llm_cleanup',
          contextBefore: '',
          contextAfter: '',
        });
      }
      if (cleanupCuts.length > 0) {
        cutResult.applied.sort((a, b) => a.start - b.start);
        console.log(
          `[clean_mode_pipeline:${jobId}] raw_video_cleanup (transcript_only): ` +
          `removed ${cleanupCuts.length} span(s), ${(cleanupRes?.totalRemovedSec ?? 0).toFixed(2)}s`,
        );
        for (const rc of cleanupCuts) {
          console.log(
            `[clean_mode_pipeline:${jobId}] [cleanup-audit] [${rc.start.toFixed(3)}, ${rc.end.toFixed(3)}] ` +
            `(${(rc.end - rc.start).toFixed(3)}s) → CUT (${rc.reason})`,
          );
        }
      }
      steps.rawVideoCleanup = {
        ms: Date.now() - stepT,
        mode: cleanupRes?.mode ?? 'transcript_only',
        removed: cleanupCuts.length,
        removedSec: Number((cleanupRes?.totalRemovedSec ?? 0).toFixed(3)),
        reasons: cleanupCuts.map((c) => c.reason).slice(0, 20),
        ...(cleanupError ? { error: cleanupError } : {}),
      };
    }

    // ── 4b-3. Best-take + end-on-CTA (transcript-only, Phase 2) — opt-in ─
    // After cleanup, optionally (a) AUTO-TRIM trailing content after the final
    // CTA so the video ENDS on the CTA (heavily gated), and (b) DETECT + LOG a
    // best-take keep/drop proposal for review. Self-detects structure from the
    // Deepgram transcript (no portal/script/#163 dependency). ONLY the gated
    // end-on-CTA trim is applied — bestTakeProposal is LOG ONLY in v1 (we review
    // proposals on real footage before trusting an auto-drop). Failure is
    // NON-FATAL: keep existing cuts and ship.
    if (opts.endOnCta || opts.bestTakeDetect) {
      stepT = stepStart('bestTakeCta');
      let btRes = null;
      let btError = null;
      try {
        btRes = await analyzeBestTakeAndCta({
          transcript,
          wordTimestamps: word_timestamps,
          sourceDuration,
          silenceMap: mergedSilenceMap,
          segmentHint: effectiveSegmentHint,   // >1 clip ⇒ no auto-trim (deferred to 1C)
          endOnCta: opts.endOnCta === true,    // bestTakeDetect-only ⇒ proposal/log, never trims
          ctaTrailBufferSec: opts.ctaTrailBufferSec,
          minRemainingSec: opts.cleanupMinRemainingSec,
        });
      } catch (err) {
        btError = err.message;
        console.warn(
          `[clean_mode_pipeline:${jobId}] best_take_cta failed (${err.message}) — ` +
          `continuing without CTA trim`,
        );
      }

      const cta = btRes?.cta ?? null;
      const proposal = btRes?.bestTakeProposal ?? { groups: [], proposedDrops: [] };
      // Merge ONLY the end-on-CTA trim into the applied cut set — SAME path as
      // the cleanup cuts above (push → sort → downstream buildKeepSegments calls
      // mergeCutSpans, which dedupes/merges it with the silence/bad-take/cleanup
      // spans). Validate the span first. NEVER apply proposal.proposedDrops —
      // best-take is LOG ONLY in v1.
      const ctaTrimCuts = [];
      for (const rc of (btRes?.removeCuts ?? [])) {
        if (typeof rc?.start !== 'number' || typeof rc?.end !== 'number') continue;
        if (!(rc.start >= 0 && rc.end > rc.start && rc.end <= sourceDuration)) continue;
        ctaTrimCuts.push(rc);
        cutResult.applied.push({
          start: rc.start,
          end: rc.end,
          category: 'cta_trim',
          reason: `end_on_cta: ${rc.reason}`.slice(0, 80),
          safety: 'safe',
          safetyReason: 'llm_cta_trim',
          contextBefore: '',
          contextAfter: '',
        });
      }
      if (ctaTrimCuts.length > 0) cutResult.applied.sort((a, b) => a.start - b.start);

      const trimApplied = ctaTrimCuts.length > 0;
      const removedSec = ctaTrimCuts.reduce((s, c) => s + (c.end - c.start), 0);
      steps.bestTakeCta = {
        ms: Date.now() - stepT,
        cta: cta
          ? { found: cta.found, end_time: Number(cta.end_time.toFixed(3)), confidence: cta.confidence }
          : null,
        trimApplied,
        removedSec: Number(removedSec.toFixed(3)),
        proposedDropCount: proposal.proposedDrops.length,
        proposedDrops: proposal.proposedDrops.slice(0, 20),
        ...(btError || btRes?.error ? { error: btError ?? btRes.error } : {}),
      };
      console.log(
        `[clean_mode_pipeline:${jobId}] [best_take_cta] ` +
        `cta@${cta?.found ? `${cta.end_time.toFixed(2)}s` : 'none'} trim=${trimApplied} ` +
        `(${removedSec.toFixed(2)}s) proposedDrops=${proposal.proposedDrops.length} (LOG ONLY)`,
      );
      for (const d of proposal.proposedDrops) {
        console.log(
          `[clean_mode_pipeline:${jobId}] [best-take-proposal] [${d.start.toFixed(2)}, ${d.end.toFixed(2)}] ` +
          `conf=${d.confidence} — ${d.reason} (LOG ONLY, not cut)`,
        );
      }
    }

    // Recompute applied-cut metadata after slate + bad-take + raw-cleanup injections.
    ...
    // PR-AP: persist cut.mp4 to Storage so /recompose-broll can swap b-roll
    // without re-cutting. Best-effort — a Storage hiccup here doesn't fail
    // the job (the final video still ships); recompose just won't be
    // available for it. Recompose route hard-fails with
    // broll_recompose_missing_intermediate if the operator later tries.
    recomposeManifest.cutDurationSec = Number(cutDuration.toFixed(3));
    let cutStorageInfo = null;
    try {
      stepT = stepStart('cutPersist');
      const cutStoragePath = `${req.output.pathPrefix}${jobId}-cut.mp4`;
      ...
      cutStorageInfo = {
        bucket: req.output.bucket,
        path: cutStoragePath,
        cutDurationSec: recomposeManifest.cutDurationSec,
      };
      steps.cutPersist = { ms: Date.now() - stepT, ok: true, storagePath: cutStoragePath };
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      warnings.push(`cut_persist_failed: ${msg.slice(0, 200)}; recompose-broll unavailable for this job`);
      steps.cutPersist = { ms: Date.now() - stepT, ok: false, error: msg.slice(0, 200) };
    }
    ...
    steps.faceDetect = {
      ...
      offsetY: Number(faceDetectResult.offsetY.toFixed(4)),
      source: faceDetectResult.source,
      detail: faceDetectResult.detail,
    };
    // PR-AP: capture face offset in the recompose manifest so /recompose-broll
    // can re-run composeFaceAndBrolls with the original framing without
    // re-running OpenCV detection.
    recomposeManifest.composeOptions.faceCropOffsetX = Number(faceDetectResult.offsetX.toFixed(4));
    recomposeManifest.composeOptions.faceCropOffsetY = Number(faceDetectResult.offsetY.toFixed(4));
    console.log(
      `[clean_mode_pipeline:${jobId}] face_detect: offsetX=${faceDetectResult.offsetX.toFixed(4)} ` +
      `offsetY=${faceDetectResult.offsetY.toFixed(4)} (${faceDetectResult.source}; ${faceDetectResult.detail})`,
    );
    ...
    if (!opts.skipBroll) {
            ...
            if (!pixabayKey) {
              ...
              for (const kw of stockKeywordsList) {
                ...
                for (const hit of searchResult.hits) {
                  ...
                  // Tier 1 anti-generic-scenery: drop pure landscape clips
                  // (tree/lake/mountain/sunset with no people/action/object
                  // anchor) before the picker sees them — unless the keyword
                  // explicitly asked for nature. Protects anchored hits like
                  // "family walking in park". See lib/stock_library_merge.js.
                  if (isGenericSceneryHit(hit.tags, kw)) {
                    warnings.push(`Pixabay px-video-${hit.id} dropped: generic scenery (tags "${hit.tags}") for non-nature query "${kw}"`);
                    continue;
                  }
                  try {
                    ...
                    // PR #193: pre-flight validation. Some Pixabay clips have
                    // stream properties that crash libx264's filter chain at
                    // compose time (real failure mode hit on Joe v5.1 ad5
                    // 2026-05-23 — first roll picked px-video-12127 which
                    // crashed with "Error while opening encoder"). Reject
                    // problematic clips here so the picker never sees them
                    // and compose never has a chance to throw.
                    const v = probe.video;
                    const reject = (reason) => {
                      console.log(`[pixabay-preflight] dropped px-video-${hit.id}: ${reason}`);
                      warnings.push(`Pixabay px-video-${hit.id} dropped at pre-flight: ${reason}`);
                    };
                    if (!v) { reject('no video stream'); continue; }
                    if (v.width === 0 || v.height === 0) { reject(`bad dims ${v.width}x${v.height}`); continue; }
                    if (v.width % 2 !== 0 || v.height % 2 !== 0) {
                      // libx264 + yuv420p requires even dimensions.
                      reject(`odd dimensions ${v.width}x${v.height} (libx264 requires even)`);
                      continue;
                    }
                    if (v.width < 480 || v.height < 480) {
                      // Upscaling small clips to 1080×{1350,1920} looks bad.
                      reject(`too small ${v.width}x${v.height} (min 480x480)`);
                      continue;
                    }
                    const allowedCodecs = new Set(['h264', 'hevc', 'h265', 'vp9', 'av1']);
                    if (!allowedCodecs.has(v.codec)) {
                      reject(`unsupported codec ${v.codec}`);
                      continue;
                    }
                    if (v.fps > 0 && (v.fps < 12 || v.fps > 60)) {
                      // Extreme frame rates break the loop+trim filter math
                      // in composeFaceAndBrolls.
                      reject(`fps ${v.fps.toFixed(2)} outside [12, 60]`);
                      continue;
                    }
                    const dur = probe.container?.duration ?? 0;
                    if (dur < 1.0) {
                      // loop=size budget becomes unreliable below 1s.
                      reject(`duration ${dur.toFixed(2)}s < 1.0s`);
                      continue;
                    }
                    // Tier 2-b (PR #201): per-asset face-aware crop offset for
                    // landscape stock. Skip portrait/square — naive center-crop
                    // is a no-op there. detectFaceOffsetX always resolves
                    // (falls back to 0.5 on failure), so composeFaceAndBrolls
                    // never breaks here. Runs ONLY for clips that pass the
                    // pre-flight validation above (#191) — saves face-detect
                    // compute on clips we'd drop anyway.
                    const isLandscape = (probe.video?.width ?? 0) > (probe.video?.height ?? 0);
                    let faceCropOffsetX = 0.5;
                    let faceCropSource = 'skipped-portrait';
                    if (isLandscape) {
                      const fd = await detectFaceOffsetX(dl.localPath, { samples: 4 });
      ...
      // ── 6e. Tier 2-a: Pexels stock as second provider ─────────────────
      // Runs after Pixabay using the SAME Gemini keywords list (no second
      // keyword-gen call). Fail-soft same way: any error → warn, continue.
      // Skipped silently when PEXELS_API_KEY is unset on the worker.
      // Skipped when caller passes pexelsEnabled: false.
      // Hits are merged into `library` with provenance:'pexels'; downstream
      // picker, exclude filter, source-balance, and diagnostics treat
      // both stock providers uniformly via the isStock predicate.
      if (opts.pexelsEnabled && stockKeywordsList.length > 0) {
        const pexelsKey = process.env.PEXELS_API_KEY?.trim();
        if (!pexelsKey) {
          // Silently skip — Tier 2-a is opt-in via env var. Don't spam
          // warnings on the legacy single-provider path.
          steps.pexelsSearch = { ms: 0, reason: 'missing_api_key', hits: 0 };
        } else {
          stepT = stepStart('pexelsSearch');
          const pexelsOutDir = getStockCacheDir(jobId, tmpDir);
          const pexelsCap = Math.max(1, Math.min(opts.pexelsMaxClips, 20));
          const pexelsCollected = [];
          for (const kw of stockKeywordsList) {
            if (pexelsCollected.length >= pexelsCap) break;
            const remaining = pexelsCap - pexelsCollected.length;
            let searchResult;
            try {
              searchResult = await searchPexelsVideos({
                query: kw,
                perPage: Math.min(5, Math.max(1, remaining)),
                minDur: 3,
                maxDur: 60,
                apiKey: pexelsKey,
              });
            ...
            if (!searchResult.ok) {
              warnings.push(
                `Pexels search '${kw}' ${searchResult.kind}` +
                (searchResult.status ? ` (${searchResult.status})` : ''),
              );
              continue;
            }
            for (const hit of searchResult.hits) {
              if (pexelsCollected.length >= pexelsCap) break;
              // Tag-relevance filter (same shape as Pixabay block).
              const queryWords = new Set(kw.toLowerCase().split(/\s+/).filter(w => w.length > 2));
              const hitTags = (typeof hit.tags === 'string' ? hit.tags : '').toLowerCase();
              const tagOverlap = [...queryWords].some(w => hitTags.includes(w));
              if (!tagOverlap) {
                warnings.push(`Pexels pexels-video-${hit.id} dropped: tags "${hit.tags}" have no overlap with query "${kw}"`);
                continue;
              }
              // Tier 1 anti-generic-scenery: applies uniformly to both providers.
              if (isGenericSceneryHit(hit.tags, kw)) {
                warnings.push(`Pexels pexels-video-${hit.id} dropped: generic scenery (tags "${hit.tags}") for non-nature query "${kw}"`);
                continue;
              }
              try {
                const dl = await downloadPexelsVideo({ hit, outDir: pexelsOutDir });
                let probe;
                try {
                  probe = await probeStreams(dl.localPath);
                } catch (probeErr) {
                  warnings.push(`Pexels pexels-video-${hit.id} ffprobe failed: ${probeErr.message ?? probeErr}`);
                  continue;
                }
                // Same pre-flight guards as Pixabay — libx264-safe stream props.
                const v = probe.video;
                const reject = (reason) => {
                  console.log(`[pexels-preflight] dropped pexels-video-${hit.id}: ${reason}`);
                  warnings.push(`Pexels pexels-video-${hit.id} dropped at pre-flight: ${reason}`);
                };
                if (!v) { reject('no video stream'); continue; }
                if (v.width === 0 || v.height === 0) { reject(`bad dims ${v.width}x${v.height}`); continue; }
                if (v.width % 2 !== 0 || v.height % 2 !== 0) { reject(`odd dimensions ${v.width}x${v.height} (libx264 requires even)`); continue; }
                if (v.width < 480 || v.height < 480) { reject(`too small ${v.width}x${v.height} (min 480x480)`); continue; }
                const allowedCodecs = new Set(['h264', 'hevc', 'h265', 'vp9', 'av1']);
                if (!allowedCodecs.has(v.codec)) { reject(`unsupported codec ${v.codec}`); continue; }
                if (v.fps > 0 && (v.fps < 12 || v.fps > 60)) { reject(`fps ${v.fps.toFixed(2)} outside [12, 60]`); continue; }
                const dur = probe.container?.duration ?? 0;
                if (dur < 1.0) { reject(`duration ${dur.toFixed(2)}s < 1.0s`); continue; }
                // Tier 2-b: per-asset face-aware crop offset for landscape stock.
                // Same pattern as the Pixabay block above. Portrait/square is
                // skipped because center-crop is a no-op for height ≥ width.
                const pxIsLandscape = (probe.video?.width ?? 0) > (probe.video?.height ?? 0);
                let pxFaceCropOffsetX = 0.5;
                let pxFaceCropSource = 'skipped-portrait';
                if (pxIsLandscape) {
                  const fd = await detectFaceOffsetX(dl.localPath, { samples: 4 });
                  pxFaceCropOffsetX = fd.offsetX;
                  pxFaceCropSource = fd.source;
                }
                pexelsCollected.push({
                  ...hit,
                  ...dl,
                  searchKeyword: kw,
                  provenance: 'pexels',                 // tags the hit for the adapter
                  sourceDurSec: probe.container?.duration ?? 0,
                  hasVideo: !!probe.video,
                  hasAudio: !!probe.audio,
                  width: probe.video?.width ?? hit.width,
                  height: probe.video?.height ?? hit.height,
                  faceCropOffsetX: pxFaceCropOffsetX,
                  faceCropSource: pxFaceCropSource,
                });
              } catch (err) {
                warnings.push(`Pexels pexels-video-${hit.id} download failed: ${err.message ?? err}`);
              }
            }
          }
          steps.pexelsSearch = {
            ms: Date.now() - stepT,
            hits: pexelsCollected.length,
            cap: pexelsCap,
            keywordsUsed: stockKeywordsList.length,
          };
          if (pexelsCollected.length > 0) {
            library = mergeStockIntoLibrary(library, pexelsCollected);
            stockHits = [...stockHits, ...pexelsCollected];
            console.log(
              `[clean_mode_pipeline:${jobId}] pexels: +${pexelsCollected.length} hits, library now ${library.length}`,
            );
          }
        }
      }

      // Tier 1 inventory exclusion: drop operator-specified weak/over-used
      // asset_ids from the merged candidate pool before the picker sees them.
      // Applies uniformly to client + stock rows; durable target is the stable
      // client-library id (stock px-video-* ids regenerate each fetch, so
      // excluding those is within-run only). Chelsea's 0:57 photo is a client
      // row → stable → the intended use. Tier 2 makes this list durable.
      if (Array.isArray(opts.brollExcludeAssetIds) && opts.brollExcludeAssetIds.length > 0) {
        const excludeSet = new Set(opts.brollExcludeAssetIds.map((id) => String(id)));
        const before = library.length;
        library = library.filter((row) => !excludeSet.has(String(row.asset_id)));
        const excluded = before - library.length;
        if (excluded > 0) {
          console.log(`[clean_mode_pipeline:${jobId}] brollExcludeAssetIds: removed ${excluded} asset(s) from candidate pool`);
          warnings.push(`broll inventory exclusion: removed ${excluded} asset(s) via brollExcludeAssetIds before picking.`);
        }
      }
      ...
      if (library.length > 0) {
        ...
        try {
          ...
          const pickResult = await pickBrollInsertions({
            ...
            // Editor-brain quality floor (Chelsea editor-mindset rewrite,
            // 2026-06-01). Composite score 0-10; picks below the floor get
            // dropped to talking-head. Default 6.0 inside pickBrollInsertions;
            // per-job override via options.brollQualityFloor.
            brollQualityFloor: opts.brollQualityFloor,
          });
          if (!pickResult.ok) {
          ...
          // Editor-brain rejected picks (composite_score below floor).
          // Surfaced into steps so the operator/QC report can see WHY a
          // weak pick didn't make it into the final edit.
          const brollRejected = Array.isArray(pickResult.rejected) ? pickResult.rejected : [];
          // Editor audit trail (2026-06-01 PR2): per-moment skip reasoning
          // Gemini emits when it returns <5 picks on a 60s+ script. Lets
          // the operator see WHY the picker chose talking-head over b-roll
          // at each non-picked moment, addressing the original Chelsea
          // feedback that the picker was self-limiting at 2-3 picks.
          const skippedMoments = Array.isArray(pickResult.skippedMoments) ? pickResult.skippedMoments : [];
          steps.brollPick = {
            ms: Date.now() - stepT,
            insertions: insertions.length,
            rejected: brollRejected.length,
            rejectedDetail: brollRejected.slice(0, 10),
            skippedMoments: skippedMoments.slice(0, 20),
            model: pickedModel,
            ok: true,
          };
          brollPickOk = true;
        ...
        if (insertions.length > 0) {
          ...
          try {
            ...
            // PR #192: surface per-asset drops (404s, unknown ids, ffprobe
            // failures, etc.) as insertion warnings so the operator can see
            // which library rows failed without grepping Railway logs.
            // Pre-PR #192 these threw and wiped the whole batch.
            for (const d of dlResult.dropped ?? []) {
              insertionWarnings.push(`broll asset dropped: ${d.asset_id} — ${d.reason}`);
            }
          ...
          // PR-AP: snapshot the normalized plan into the recompose manifest.
          // Each insertion already carries asset_id + downloadUrl + provenance
          // (set by downloadBrollAssets), so the recompose route can resolve
          // a replacement by ID + re-download from the same source.
          recomposeManifest.insertions = normalizedInsertions.map((ins) => ({
            startSec: Number(ins.startSec.toFixed(3)),
            endSec: Number(ins.endSec.toFixed(3)),
            asset_id: ins.asset_id,
            downloadUrl: ins.downloadUrl ?? ins.url ?? null,
            provenance: ins.provenance ?? null,
            visual_concept: ins.visual_concept ?? null,
            reason: ins.reason ?? null,
            matchedPhrase: ins.matchedPhrase ?? null,
            match_type: ins.match_type ?? null,
            // Tier 2-b: preserve per-asset crop offset so /recompose-broll
            // can reproduce the same framing on a surgical swap without
            // re-running face detection on the kept insertions.
            faceCropOffsetX: typeof ins.faceCropOffsetX === 'number' ? ins.faceCropOffsetX : 0.5,
          }));
          ...
          if (normalizedInsertions.length > 0) {
            ...
            try {
              composedSegmentMeta = await composeFaceAndBrolls({
                ...
                faceCropOffsetY: faceDetectResult.offsetY,
                outputWidth: opts.outputWidth,
                outputHeight: opts.outputHeight,
              });
              ...
              const msg = err instanceof Error ? err.message : String(err);
              const stderrTail = err && err.stderr ? String(err.stderr).slice(-800) : '(no stderr)';
              console.log(`[clean_mode_pipeline:${jobId}] broll_compose_failed: ${msg.slice(0, 200)}\n--- ffmpeg stderr (last 800B) ---\n${stderrTail}`);
              ...
              // Use [] (not null) so downstream code that calls .length /
              // .map on composedSegmentMeta still works — the post-banner
              // TypeError "Cannot read properties of null (reading 'length')"
              // was caused by null deref at line 1762/1863/2034/2054 etc.
              composedSegmentMeta = [];
            }
          }
        } else {
    ...
    // ── 9b. Reframe to output dimensions when compose didn't run ─────────
    // compose (the !skipBroll path above) is the ONLY step that scales +
    // face-crops the cut to opts.outputWidth×opts.outputHeight. When it did
    // NOT run — skipBroll (every ad since portal #352 turned b-roll off), the
    // picker returned 0 insertions, normalize dropped them all, or compose
    // threw — `videoForSubtitles` is still the raw source-dimension cut.mp4,
    // so the output would ship at the SOURCE aspect/size (confirmed 2026-06-09:
    // a 720×1280 9:16 selfie shipped as a "1080×1350 ad" but was actually
    // 720×1280 — no crop). Apply the SAME face-aware reframe here by reusing
    // composeFaceAndBrolls with ZERO insertions (it degrades to one face
    // segment = a pure scale+crop). Gated on a real dimension mismatch so an
    // already-correct source is a byte-identical no-op (no extra re-encode).
    // NON-FATAL: on any failure keep the source-dimension cut and ship.
    if (videoForSubtitles === cutPath) {
      stepT = stepStart('reframe');
      let reframed = false;
      let reframeError = null;
      let srcDimsLabel = null;
      try {
        const probe = await probeStreams(cutPath);
        const sw = probe.video?.width ?? 0;
        const sh = probe.video?.height ?? 0;
        srcDimsLabel = `${sw}x${sh}`;
        const needsReframe = sw !== opts.outputWidth || sh !== opts.outputHeight;
        if (needsReframe) {
          await composeFaceAndBrolls({
            facePath: cutPath,
            brolledPath: reframedPath,
            insertions: [],                       // zero insertions ⇒ pure reframe
            totalDuration: cutDuration,
            faceCropOffsetX: faceDetectResult.offsetX,
            faceCropOffsetY: faceDetectResult.offsetY,
            outputWidth: opts.outputWidth,
          ...
          streamSync.reframed = await verifyMP4StreamSync(reframedPath);
          videoForSubtitles = reframedPath;
          videoForSubtitlesDuration = await getDuration(reframedPath);
          reframed = true;
          console.log(
            `[clean_mode_pipeline:${jobId}] reframe: ${srcDimsLabel} → ` +
            `${opts.outputWidth}x${opts.outputHeight} (no b-roll, face-aware crop)`,
          );
        } else {
          console.log(`[clean_mode_pipeline:${jobId}] reframe: source already ${srcDimsLabel} — no-op`);
        }
      } catch (err) {
        reframeError = err instanceof Error ? err.message : String(err);
        console.log(`[clean_mode_pipeline:${jobId}] reframe_failed: ${reframeError.slice(0, 200)} — keeping source-dimension cut`);
        warnings.push(`reframe_failed: ${reframeError.slice(0, 200)}; output stays at source dimensions`);
      }
      steps.reframe = {
        ms: Date.now() - stepT,
        reframed,
        sourceDims: srcDimsLabel,
        targetDims: `${opts.outputWidth}x${opts.outputHeight}`,
        ...(reframeError ? { error: reframeError } : {}),
      };
    }
    ...
    if (introHook.enabled) {
      ...
      try {
        ...
        hookResult = await generateHookText({
          ...
          model: opts.model,
          // 2026-06-10: brand stylization preservation. EnableSNP needs
          // "ENABLE" all-caps; without this the LLM's Title-Case rule
          // strips it to "Enable" (Chelsea flagged Tue Jun 9 reel).
          brandTermsAllCaps: opts.brandTermsAllCaps,
          // 2026-06-10: deterministic title-fallback when both Pro and
          // Flash hook calls fail validation. Without this, Wed Jun 10
          // + Fri Jun 12 shipped with no overlay because the LLM kept
          // generating invalid hooks. Now we derive from the content
          // item title (same source the portal uses for slateHint).
          titleForHookFallback: opts.titleForHookFallback,
        });
      } catch (err) {
        hookResult = {
          ok: false,
      ...
      if (!hookResult.ok) {
        ...
        try {
          ...
          // PR-AP: snapshot intro hook state for recompose so a later
          // /recompose-broll re-applies the same overlay.
          recomposeManifest.introHook = {
            applied: true,
            hookText: hookResult.hookText,
            durationSec: introHook.durationSec,
          };
    ...
    if (bgmActive) {
      ...
      if (bgmLocalPath) {
        ...
        try {
          ...
          // PR-AP: snapshot BGM state for recompose. The track can be
          // re-downloaded by ID from Jamendo (their library is stable).
          // appliedVolumeLinear is the exact knob the new mix should use
          // so the recomposed output sounds identical to the original.
          recomposeManifest.bgm = {
            enabled: true,
            jamendoTrackId: bgmTrackMeta?.id ?? null,
            downloadUrl: bgmTrackMeta?.downloadUrl ?? null,
            durationSec: bgmTrackMeta?.durationSec ?? null,
            appliedVolumeLinear: bgmMixMeta.appliedVolumeLinear,
            fadeSec,
            extraReductionDb: opts.bgmVolumeDb ?? 0,
          };
    ...
    // ── 14c. PR-AP: persist recompose manifest + insertions.json for /recompose-broll ──
    //
    // Captures the complete post-cut state (insertions plan, compose options,
    // intro hook applied, banner config, bgm track + applied volume, caption
    // style) so a future surgical b-roll swap can re-run only:
    //   compose → overlay → banner → subtitle burn → bgm mix
    // without re-cutting, re-picking, or re-transcribing. cut.mp4 was
    // uploaded earlier (step 5b). All three must be present for
    // /recompose-broll to succeed; if any are missing the route returns
    // broll_recompose_missing_intermediate.
    if (cutStorageInfo) {
      stepT = stepStart('recomposeManifestUpload');
      try {
        const manifestPath = `${req.output.pathPrefix}${jobId}-recompose-manifest.json`;
        const insertionsPath = `${req.output.pathPrefix}${jobId}-insertions.json`;
        const manifestBuf = Buffer.from(JSON.stringify(recomposeManifest, null, 2));
        const insertionsBuf = Buffer.from(JSON.stringify(recomposeManifest.insertions, null, 2));
        const manifestLocal = join(tmpDir, `${jobId}-recompose-manifest.json`);
        const insertionsLocal = join(tmpDir, `${jobId}-insertions.json`);
        writeFileSync(manifestLocal, manifestBuf);
        writeFileSync(insertionsLocal, insertionsBuf);
        await uploadToStorage({
          bucket: req.output.bucket,
          path: manifestPath,
          filePath: manifestLocal,
          contentType: 'application/json',
        });
        await uploadToStorage({
          bucket: req.output.bucket,
          path: insertionsPath,
          filePath: insertionsLocal,
          contentType: 'application/json',
        });
        recomposeAssets = {
          cutStorageBucket: cutStorageInfo.bucket,
          cutStoragePath: cutStorageInfo.path,
          cutDurationSec: cutStorageInfo.cutDurationSec,
          manifestStoragePath: manifestPath,
          insertionsStoragePath: insertionsPath,
          insertionsCount: recomposeManifest.insertions.length,
        };
        steps.recomposeManifestUpload = {
          ms: Date.now() - stepT,
          ok: true,
          manifestBytes: manifestBuf.length,
          insertionsBytes: insertionsBuf.length,
        };
      } catch (err) {
        const msg = err?.message ?? String(err);
        warnings.push(`recompose-manifest upload skipped (non-fatal): ${msg}`.slice(0, 600));
        steps.recomposeManifestUpload = { ms: Date.now() - stepT, ok: false, error: msg };
      }
    }
    ...
    const insertionsDetail = normalizedInsertions.map((ins) => {
      ...
      return {
        ...
        // Tier 1 diagnostics: Gemini's editorial rationale per insertion so a
        // client complaint at a timestamp maps directly to an asset_id + why.
    ...
    // Tier 1: deterministic per-insertion diagnostic log. One line per
    // insertion joining timestamp + asset + source + Gemini reason, so future
    // feedback is actionable without rewatching the video and guessing which
    // asset is where. (Operators grep `[broll-diag:<jobId>]` in Railway logs.)
    {
      const { lines: brollDiagLines } = formatInsertionDiagnostics(normalizedInsertions);
      for (const line of brollDiagLines) {
        console.log(`[broll-diag:${jobId}] ${line}`);
      }
    }
    ...
    return {
      ...
      insertions: {
        ...
        // insertion list split between client library and stock.
        // 2026-06-01 fix: stockCount now includes Pexels alongside Pixabay
        // (Tier 2-a added Pexels as a second stock provider; the old filter
        // only counted `pixabay` and undercounted stock — visible as
        // "0 client + 0 stock" in QC reports despite N Pexels picks).
        clientCount: insertionsDetail.filter((d) => d.source === 'client').length,
        stockCount: insertionsDetail.filter((d) => d.source === 'pixabay' || d.source === 'pexels').length,
        // Per-provider breakdown for debugging; keeps backward compat (any
        // existing reader of stockCount still works; new readers can split).
      ...
      repatchAssets,
      // PR-AP: cut.mp4 + insertions.json + recompose-manifest.json storage
      // paths. Surfaced so the portal callback / sync caller can record
      // them on the content row, enabling later /recompose-broll calls
      // without re-running the full pipeline. Null when cut.mp4 upload
      // failed (recompose unavailable for this job).
      recomposeAssets,
    ...
    const errTerminal = isNoSpeechError(err?.message ?? '');
    return {
      jobId,
      error: {
        step: errStep,
        message: err?.message ?? String(err),
        // `terminal: true` → re-running this exact source can't succeed
        // (e.g. no speech in the audio). The route forwards it to the
        // portal callback so the hourly retry sweep can skip it instead of
        // burning MAX_RETRIES attempts + Deepgram credit. Only set when
        // true; omitted otherwise to keep existing failure envelopes byte-
        // identical for all other (retryable) error steps.
        ...(errTerminal ? { terminal: true } : {}),
      },
      ...
      recomposeAssets,                                 // PR-AP (null when cut.mp4 upload failed)
...
/**
 * Is this error TERMINAL for the given source — i.e. re-running the exact
 * same job can't succeed? Currently the one terminal case is "no speech in
 * the source audio" (Deepgram returns 0 words). It's a property of the
 * uploaded file, not a transient/infra hiccup, so retrying just re-spends
 * Deepgram credit to fail identically and re-pings Slack every hour.
 *
 * Matches both the new human-readable message and the legacy
 * "Deepgram returned 0 words" phrasing (kept as a substring in the throw)
 * so this stays true regardless of which message variant is in flight.
 *
 ...
 * @returns {boolean}
 */
export function isNoSpeechError(message) {
  return /No speech detected|Deepgram returned 0 words/i.test(message ?? '');
}
...
export async function downloadBrollAssets(insertions, library, brollDir, opts = {}) {
  ...
  const dropped = [];              // PR #192: per-asset drop log [{ asset_id, reason }]
  const heicConvert = {
    attempted: 0,
    converted: 0,
    failed: 0,
  ...
  for (const ins of insertions) {
    // PR #192: per-asset fail-soft. Each insertion runs inside its own
    // try/catch so a single dead URL / unknown asset_id / failed ffprobe
    // drops just THAT asset instead of throwing the whole batch. Previously
    // (Saturday 2026-05-23 incident with SupportED Joe v5.1 ad batch) one
    // dead row in marketing.broll_library 404'd, this function threw, the
    // whole-call catch in the orchestrator wiped all 17 picked insertions,
    // and every render shipped as talking-head-only.
    try {
      ...
      if (!row) {
        ...
        // If multiple match, drop this asset (don't silently choose).
        ...
        if (typeof requested === 'string' && requested.length >= 4) {
          ...
          if (matches.length === 1) {
            ...
            throw new Error(
              `Ambiguous asset_id=${requested} (matches ${matches.length} rows: ${matches.map((r) => r.asset_id).join(', ')})`,
            );
          }
        }
        if (!row) {
          throw new Error(`Unknown asset_id=${ins.asset_id} (not in library)`);
        }
      } else {
      ...
      if (!downloadUrl) {
        throw new Error(
          `Broll ${ins.asset_id} has neither file_url nor storage_url (drive_file_id-only not supported)`,
        );
      }
      ...
      if (res.status < 200 || res.status >= 300) {
        ...
        throw new Error(`Download ${res.status} for ${downloadUrl}: ${body}`);
      }
      ...
      if (shouldConvert) {
        ...
        try {
          ...
          // (Pre-PR #192 this was the only fail-soft path; now all 5 throws
          // route through the outer try/catch instead.)
          continue;
        }
      }
      ...
      const probe = await probeStreams(assetPath);
      out.push({
        ...ins,
        localPath: assetPath,
        bytes,
      ...
      const reason = (err?.message ?? String(err)).slice(0, 240);
      dropped.push({ asset_id: ins.asset_id, reason });
      console.log(`[downloadBrollAssets] dropped asset_id=${ins.asset_id}: ${reason}`);
      // continue with next insertion — do NOT rethrow
    }
  }
  return { assets: out, heicConvert, assetIdResolutions, dropped };
}
 ...
 *   a no-op.
 * @param {number} [args.faceCropOffsetY=0.5]  fraction in [0,1] — VERTICAL
 *   crop center for face video segments. 0.5 = center crop. Previously the
 *   y-crop was pinned to 0 (top), which clipped a low-framed speaker's face
 *   when a tall 9:16 source was cropped to a 4:5 ad. No-op for same-aspect
 *   reframes (no vertical crop room). Brolls keep the top crop.
 * @returns {Promise<Array<{ asset_id, requestedDurSec, sourceDurSec, actualSegmentDurSec, paddingApplied }>>}
 */
export async function composeFaceAndBrolls({
  facePath,
  brolledPath,
  insertions,
  totalDuration,
  ...
  segments.forEach((seg, i) => {
    ...
    if (seg.kind === 'face') {
      ...
      // Face-aware VERTICAL offset (was hardcoded `:0` = top crop). On a tall
      // 9:16 selfie cropped to a 4:5 ad, the old top crop kept the background
      // and clipped a low-framed face; buildCropYExpression centers the crop on
      // the median face-center Y instead. No-op for same-aspect reframes.
      const cropYExpr = buildCropYExpression(faceCropOffsetY, outputHeight);
      parts.push(
        `[0:v]trim=${seg.startSec.toFixed(3)}:${seg.endSec.toFixed(3)},` +
        `setpts=PTS-STARTPTS,` +
        `scale=${outputWidth}:${outputHeight}:force_original_aspect_ratio=increase,` +
        `crop=${outputWidth}:${outputHeight}:${cropXExpr}:${cropYExpr},` +
      ...
      // Tier 2-b: per-asset face-aware horizontal crop for b-roll segments.
      // Default 0.5 = naive center crop = pre-Tier-2-b behavior (portrait
      // iPhone clients + landscape with no face detected fall back here).
      // When the pipeline detected a face on landscape stock and set
      // ins.faceCropOffsetX, we use it to shift the crop window so the
      // subject's face stays in frame. Repro: Thursday 75.7s 2026-05-28 —
      // a Pexels woman-at-laptop with face on the right was being cropped
      // off; face-aware offset fixes that.
      const brollOffsetX = typeof ins.faceCropOffsetX === 'number' ? ins.faceCropOffsetX : 0.5;
      ...
      parts.push(
        ...
        `scale=${outputWidth}:${outputHeight}:force_original_aspect_ratio=increase,` +
        `crop=${outputWidth}:${outputHeight}:${brollCropXExpr}:0,` +
        `setsar=1,format=yuv420p[${outLabel}]`,
      );
      segmentMeta.push({
        asset_id: ins.asset_id,
