...
export async function runCleanModePipeline(req) {
  ...
  try {
    ...
    cutResult = detectAndClassifyCuts(word_timestamps, {
      ...
      // PR #105: clean-mode opts in to skipping Scribe "silence ghost" words
      // during the word-boundary clamp. Justine's 1:11–1:13 fixture had
      // Scribe report "The" as a single 2.71s word [87.24, 89.95] that
      // mostly overlapped silencedetect [87.31, 89.75] — the clamp would
      // pull cut.end back to 87.19 and undo the merged silence span PR #102
      // had just collected. Setting this true with externalSilences supplied
      // lets the clamp ignore that word so the merge survives. Default off
      // in cut_detection.js to preserve the broader test parity suite.
