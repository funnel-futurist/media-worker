...
import { detectSlate } from './slate_detect.js';
  ...
  try {
    ...
    if (slateMetadata) {
      cutResult.applied.unshift({
        start: 0,
        end: slateMetadata.end,
        category: 'silence',
        reason: `slate_intro (llm): ${(slateMetadata.transcribed_text || '').slice(0, 80)}`.trim(),
        safety: 'safe',
  ...
  segments.forEach((seg, i) => {
    ...
    if (seg.kind === 'face') {
      parts.push(
        ...
        `setpts=PTS-STARTPTS,` +
        // PR #111: fill-crop instead of letterbox-pad. See composeFaceAndBrolls
        // header for the rationale + trade-offs (off-center subjects = follow-up).
