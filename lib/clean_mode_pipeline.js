    ...
    if (introHook.enabled) {
      ...
      if (!hookResult.ok) {
        ...
        try {
          ...
          warnings.push(`intro card render skipped: ${msg}`.slice(0, 240));
          console.log(`[clean_mode_pipeline:${jobId}] intro hook SKIPPED at renderIntroCard: ${msg.slice(0, 240)}`);
        }
        ...
        if (renderOk) {
          ...
          try {
            ...
            const msg = err instanceof Error ? err.message : String(err);
            warnings.push(`intro concat skipped: ${msg}`.slice(0, 240));
            console.log(`[clean_mode_pipeline:${jobId}] intro hook SKIPPED at concatIntro: ${msg.slice(0, 240)}`);
          }
