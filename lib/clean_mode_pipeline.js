...
async function composeFaceAndBrolls({ facePath, brolledPath, insertions, totalDuration }) {
  ...
  segments.forEach((seg, i) => {
    ...
    if (seg.kind === 'face') {
      ...
      if (typeof ins.sourceDurSec !== 'number' || !Number.isFinite(ins.sourceDurSec) || ins.sourceDurSec <= 0) {
        throw new Error(
      ...
      // Tolerance: treat "essentially equal" as trim-only to avoid a redundant
      // loop pass when asset_dur is within ~50ms of dur.
      const needsLoop = ins.sourceDurSec + 0.05 < dur;
