...
export async function runCleanModePipeline(req) {
  ...
  try {
    ...
    if (opts.bannerEnabled && opts.bannerConfig?.text) {
      ...
      const banneredPath = join(workDir, 'bannered.mp4');
    ...
    if (!opts.skipSubtitles) {
      ...
      const burnResult = await writeAssAndBurn({
        ...
        assOpts: {
          ...
          // When banner is active, push captions to the bottom caption zone
          // (y=1050-1350 in the layout spec). MarginV=150 at playResY=1350
          // places the caption baseline at ~y=1200 (center of caption zone),
          // well below the speaker's face in the content zone (y=200-1050).
          // When banner is NOT active, fall back to the default 27.1% ratio.
          ...(bannerApplied ? { marginV: 150 } : {}),
        },
      });
