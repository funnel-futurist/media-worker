...
import { renderIntroCard } from './intro_card_render.js';
import {
...
export function buildPipelineOpts(req) {
  return {
    ...
    // genuinely doesn't. Coupled with PR-K's 6–8s insertion duration
    // (target ~7s), 0.55 floor on a 120s reel translates to ~9 picks
    // worth of coverage time, with room for the picker to add more when
    // warranted. Per-job override still wins.
    ...
    brollClientPreference: req.options?.brollClientPreference,
    brollMaxClientCount: req.options?.brollMaxClientCount,
    // PR-K: per-job b-roll insertion duration controls. Defaults are
    // applied at the use sites (broll_picker.buildPrompts and
    // normalizeInsertions) so undefined here means "use the 6/7/8s
...
export async function runCleanModePipeline(req) {
  ...
  try {
    ...
    // ── 5.5 / 5.6 / 5.7: PR-L intro hook + card + concat ────────────────
    // When opts.introHookEnabled is true:
    //   5.5 hookGenerate — Gemini Pro generates an ≤8-word hook from the
    //                       transcript (anti-fabrication rules in the prompt;
    //                       see lib/hook_generate.js). Skip-and-warn on any
    //                       failure — no generic fallback per Shannon's spec.
    //   5.6 introCardRender — ffmpeg drawtext renders a 5s polished card.
    //   5.7 concatIntro     — ffmpeg filter_complex concat re-encodes
    //                          intro.mp4 + cut.mp4 → cut_with_intro.mp4.
    //                          Using filter_complex (not concat demuxer)
    //                          tolerates codec/sample-rate mismatches between
    //                          the intro card and the source-derived cut.mp4
    //                          at the cost of one extra re-encode.
    //
    // composeBasePath/composeBaseDuration carry the picked base through to
    // compose. They default to cutPath/cutDuration (legacy behavior) and
    // get swapped to cut_with_intro.mp4 + cutDuration+intro when the hook
    // applies cleanly.
    //
    // The broll picker continues to run on the cut.mp4 timeline (it never
    // sees the intro). After normalize, the orchestrator shifts each
    // insertion forward by introDurationSec via offsetInsertions() so
    // compose places brolls on the cut_with_intro timeline correctly.
    // Subtitle remap also shifts every word event forward by the same
    // amount (PR-L extension of remapWordsThroughCuts).
    const cutWithIntroPath = join(tmpDir, 'cut_with_intro.mp4');
    /** @type {{enabled: boolean, applied: boolean, hookText: string | null, durationSec: number, skipReason: string | null, attribution: string | null}} */
    const introHook = {
    ...
    let composeBasePath = cutPath;
    let composeBaseDuration = cutDuration;

    // PR-T: explicit log when the intro hook block is entered. Without this
    // the orchestrator made it impossible to tell from Railway logs whether
    // (a) opts.introHookEnabled was false (block didn't run), (b) Gemini
    // returned malformed JSON (silent invalid_json skip), or (c) Gemini
    // returned out-of-spec text (silent invalid_hook_text skip). PR-L
    // verification was effectively blind. Each skip path now console.logs.
    if (introHook.enabled) {
      console.log(`[clean_mode_pipeline:${jobId}] intro hook enabled — running step 5.5 hookGenerate`);
      // 5.5 hookGenerate
      const hookGenStepT = Date.now();
      let hookResult;
      try {
        const remappedForHook = remapWordsThroughCuts(word_timestamps, appliedCuts);
        const transcriptText = remappedForHook.map((w) => w.word).join(' ');
      ...
      steps.hookGenerate = {
        ...
        ok: hookResult.ok,
        ...(hookResult.ok ? { hookText: hookResult.hookText } : { reason: hookResult.reason }),
      };
      if (!hookResult.ok) {
        ...
        // PR-T: surface skip to Railway stdout so we can diagnose without
        // waiting for the response envelope (which we never see when the
        // pipeline fails downstream — broll picker timeout etc.).
        ...
        // 5.6 renderIntroCard
        const renderStepT = Date.now();
        const introCardPath = join(tmpDir, 'intro.mp4');
        let renderOk = false;
        try {
          ...
          renderOk = true;
          console.log(`[clean_mode_pipeline:${jobId}] intro card rendered → ${introCardPath}`);
        } catch (err) {
          introHook.skipReason = 'render_failed';
          // PR-X: bumped truncation 240 → 1200 so ffmpeg's stderr (the
          // useful diagnostic) reaches Railway logs. Pre-PR-X this was
          // 240 chars which only showed the command itself, not what
          // ffmpeg said FAILED. Today's jobId c88d5d3f was the smoking
          // gun — log ended mid-command at "Coordination" with no
          // stderr visible. 1200 is enough for the typical drawtext
          // error block.
          const msg = err instanceof Error ? err.message : String(err);
          warnings.push(`intro card render skipped: ${msg}`.slice(0, 1200));
          console.log(`[clean_mode_pipeline:${jobId}] intro hook SKIPPED at renderIntroCard: ${msg.slice(0, 1200)}`);
        }
        steps.introCardRender = { ms: Date.now() - renderStepT, ok: renderOk };

        // 5.7 concatIntro (filter_complex re-encode — robust against
        // codec/sample-rate mismatches between intro.mp4 and cut.mp4)
        if (renderOk) {
          const concatStepT = Date.now();
          let concatOk = false;
          try {
            const concatCmd =
              `ffmpeg -y -i "${introCardPath}" -i "${cutPath}" ` +
            ...
            concatOk = true;
            // A/V sync gate on the concatenated output. If the concat
            // produced a desynced file we want to know BEFORE compose
            // amplifies the damage.
            streamSync.cutWithIntro = await verifyMP4StreamSync(cutWithIntroPath);
            composeBasePath = cutWithIntroPath;
            composeBaseDuration = cutDuration + introHook.durationSec;
            introHook.applied = true;
            console.log(`[clean_mode_pipeline:${jobId}] intro hook APPLIED — composeBase shifted to cut_with_intro.mp4 (+${introHook.durationSec.toFixed(1)}s offset)`);
          } catch (err) {
            introHook.skipReason = 'concat_failed';
            // PR-X: same truncation bump as the render-failed branch above.
            const msg = err instanceof Error ? err.message : String(err);
            warnings.push(`intro concat skipped: ${msg}`.slice(0, 1200));
            console.log(`[clean_mode_pipeline:${jobId}] intro hook SKIPPED at concatIntro: ${msg.slice(0, 1200)}`);
          }
          steps.concatIntro = { ms: Date.now() - concatStepT, ok: concatOk };
        }
      }
    }

    // ── Default: composeBasePath is the input to the next step. Skip flags
    //     reroute below. ─────────────────────────────────────────────
    ...
    let videoForSubtitles = composeBasePath;
    let videoForSubtitlesDuration = composeBaseDuration;
    ...
    if (!opts.skipBroll) {
        ...
        if (!coverage.trigger) {
          ...
          if (!keywordsResult.ok) {
            ...
            if (!pixabayKey) {
              ...
              const cap = Math.min(opts.pixabayMaxClips, coverage.fetchCount);
              const collected = [];
      ...
      if (library.length > 0) {
        ...
        // PR #130: per-job clientPreference override. Defaults to 'balanced'
        // (current AI-blend behavior); 'minimal' injects a stronger stock-bias
        // clause into the picker prompt for clients whose library is mostly
        // static photos (Phil). Other values fall through to 'balanced'.
        ...
        const maxStockRatio = Number(
          opts.brollMaxStockRatio ?? process.env.BROLL_MAX_STOCK_RATIO ?? 0.55,
        );
        ...
        if (insertions.length > 0) {
          ...
          const downloads = dlResult.assets;
          ...
          //
          // IMPORTANT (PR-L): normalize still clamps to cutDuration (cut.mp4
          // timeline), NOT composeBaseDuration. The picker ran on cut.mp4
          // and emits insertions in [0, cutDuration]. We shift them onto
          // the cut_with_intro timeline AFTER normalize via offsetInsertions
          // below. Doing the clamp on cut.mp4 first means a picker pick of
          // [99, 105] on a 100s cut.mp4 still gets clamped to [99, 100],
          // then the PR-K min-duration floor catches the 1s window — same
          // behavior as without intro.
          normalizedInsertions = normalizeInsertions(downloads, cutDuration, insertionWarnings, {
            brollMinDurationSec: opts.brollMinDurationSec,
          });
          // PR-L: shift insertions onto the cut_with_intro timeline if the
          // intro hook applied. Picker timeline is still cut.mp4; compose
          // timeline is cut_with_intro.mp4 (= cutDuration + introDurationSec).
          if (introHook.applied) {
            normalizedInsertions = offsetInsertions(normalizedInsertions, introHook.durationSec);
          }
          ...
          if (normalizedInsertions.length > 0) {
            ...
            // PR-L: facePath + totalDuration come from composeBasePath /
            // composeBaseDuration — these are cutPath/cutDuration by
            // default, or cut_with_intro.mp4 / (cutDuration + intro) when
            // the intro hook applied.
            stepT = stepStart('compose');
            composedSegmentMeta = await composeFaceAndBrolls({
              facePath: composeBasePath,
              brolledPath,
              insertions: normalizedInsertions,
              totalDuration: composeBaseDuration,
    ...
    if (!opts.skipSubtitles) {
      ...
      // PR-L: when intro hook applied, every subtitle word event shifts
      // forward by introDurationSec so the .ass file aligns with the
      // speaker in the final composition (which starts at t=introDurationSec
      // after the prepended intro card). The PR #127 cut-overlap-merge
      // fix INSIDE remapWordsThroughCuts is unchanged — the offset is
      // purely additive on top.
      stepT = stepStart('subtitleGen');
      const remappedWords = remapWordsThroughCuts(word_timestamps, appliedCuts, {
        introOffsetSec: introHook.applied ? introHook.durationSec : 0,
      });
      ...
      const burnResult = await writeAssAndBurn({
        ...
        assOpts: { playResX: opts.outputWidth, playResY: opts.outputHeight },
      });
    ...
    return {
      ...
      // PR-L: intro hook diagnostic block. When enabled and applied,
      // surfaces the AI-generated hookText + durationSec; on skip,
      // surfaces the skipReason so an operator can spot Gemini failures
      ...
      streamSync: {
        ...
        cutWithIntro: streamSync.cutWithIntro ?? null,    // PR-L: present only when intro applied
 ...
 * @param {object} [opts]
 * @param {number} [opts.brollMinDurationSec=6.0]  PR-K: floor for the
 ...
 *   ordering matters: a picker pick of [120, 128] that clamps to [120, 121]
 *   because cutDuration=121 should be dropped (1s after clamp < 6s floor),
 *   not kept because it was 8s before clamp.
...
export function normalizeInsertions(rawInsertions, cutDuration, warnings, opts = {}) {
  ...
  for (const ins of rawInsertions) {
    ...
    // each visual. Picker prompt aims for ~7s already; this is the safety
...
/**
 * Shift each insertion's startSec/endSec forward by `offsetSec` (PR-L,
 * 2026-05-13). Pure function so the timeline math is unit-testable.
 *
 * Use case: the broll picker runs against the cut.mp4 timeline (it never
 * sees the intro card prepended by PR-L). After picking + normalize, the
 * orchestrator shifts each insertion forward by introDurationSec so the
 * compose step (which operates on the cut_with_intro timeline) places
 * brolls at the speaker section, not over the intro card.
 *
 * Negative or zero offsets are a no-op (returns a shallow copy). The
 * function never mutates its input.
 *
 * @param {Array<{ startSec: number, endSec: number, ... }>} insertions
 * @param {number} offsetSec
 * @returns {Array}  new array with shifted timestamps
 */
export function offsetInsertions(insertions, offsetSec) {
  if (!Array.isArray(insertions)) return [];
  if (typeof offsetSec !== 'number' || !Number.isFinite(offsetSec) || offsetSec <= 0) {
    return insertions.map((i) => ({ ...i }));
  }
  return insertions.map((i) => ({
    ...i,
    startSec: i.startSec + offsetSec,
    endSec: i.endSec + offsetSec,
  }));
}
