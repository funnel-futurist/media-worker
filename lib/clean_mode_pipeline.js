...
export async function runCleanModePipeline(req) {
  ...
  try {
    ...
    const inputValidation = await validateSourcePairing({
    ...
    let insertions = [];                  // raw picker output
    let normalizedInsertions = [];        // post-clamp/sort/dedupe (used for compose)
    let composedSegmentMeta = [];         // per-segment metadata returned by composeFaceAndBrolls
    ...
    return {
      ...
      contactSheetUrl,
      // PR #101: read-only diagnostics for cut-tuning investigations.
      // silenceMap is the FULL silencedetect output (every span found in
      // source.mp4 audio at -35dB / >=0.6s). Compare against
      // cuts.appliedDetail to see which spans became cuts vs which were
      // dropped/merged/skipped. Used to inform whether a tuning PR
      // (lower noiseDb, lower minGap, etc) is warranted — without
      // making a cut-aggressive change blindly.
      diagnostics: {
...
async function fetchBrollLibrary(clientId) {
  ...
  const droppedCount = rawRows.length - usableRows.length;
  if (droppedCount > 0) {
    console.log(
      `[clean_mode_pipeline] broll_library: ${rawRows.length} rows for client=${clientId}, ` +
      `${droppedCount} dropped (no file_url and no storage_url), ${usableRows.length} usable`,
    );
  }
 ...
 * @param {Object} [opts]
 * @param {number} [opts.toleranceSec=0.1]
...
async function verifyMP4StreamSync(filePath, opts = {}) {
  const toleranceSec = typeof opts.toleranceSec === 'number' ? opts.toleranceSec : 0.1;
