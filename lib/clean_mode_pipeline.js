...
import { shouldFetchStock, mergeStockIntoLibrary } from './stock_library_merge.js';
import { getStockCacheDir } from './stock_cache.js';
// PR-B: BGM (background music) — Pixabay-Music search + adaptive LUFS mix.
import { searchPixabayMusic, downloadPixabayMusic } from './pixabay_music.js';
...
export async function runCleanModePipeline(req) {
  ...
  let library = [];
  let clientLibraryCount = 0;          // PR-A: tracked for response.insertions
  ...
  let bgmTrackMeta = null;             // pixabay-music download metadata
  ...
  try {
    ...
    if (!opts.skipBroll) {
      ...
      clientLibraryCount = library.length;     // PR-A: snapshot before stock merge
        ...
        if (!coverage.trigger) {
          ...
          if (!keywordsResult.ok) {
            ...
            if (!pixabayKey) {
              ...
              const cap = opts.pixabayMaxClips;
              const collected = [];
    ...
    if (bgmActive) {
      ...
      if (!selectResult.ok) {
        ...
        // ── 11c. bgmFetch (Pixabay-Music search + download) ───────────
        stepT = stepStart('bgmFetch');
        const pixabayKey = process.env.PIXABAY_API_KEY?.trim();
        if (!pixabayKey) {
          bgmSkipReason = 'bgm_fetch_missing_api_key';
          warnings.push(
            'BGM skipped: PIXABAY_API_KEY is not set on Railway. Pixabay-Music ' +
            'is the BGM source for M2; set the key or set bgmEnabled=false.',
          );
          ...
          let searchResult;
          try {
            searchResult = await searchPixabayMusic({
          ...
          if (!searchResult.ok) {
            ...
            warnings.push(`BGM skipped: pixabay-music ${searchResult.kind}`);
            steps.bgmFetch = { ms: Date.now() - stepT, ok: false, kind: searchResult.kind };
          } else {
            // Pick the first hit (Pixabay search returns by relevance).
            const pick = searchResult.hits[0];
            try {
              const dl = await downloadPixabayMusic({ hit: pick, outDir: tmpDir });
              bgmLocalPath = dl.localPath;
              bgmTrackMeta = { ...pick, ...dl };
              steps.bgmFetch = { ms: Date.now() - stepT, ok: true, hits: searchResult.hits.length, pickedId: pick.id };
            } catch (err) {
              bgmSkipReason = `bgm_fetch_download:${err.message}`;
              warnings.push(`BGM skipped: pixabay-music download failed — ${err.message}`);
      ...
      if (bgmLocalPath) {
        ...
        try {
          ...
          const mixOut = await mixBgmIntoVideo({
            ...
            bgmSourceDurSec: bgmTrackMeta.duration ?? subtitleVideoDur,
      ...
      bgmConfig = {
        ...
        track: bgmTrackMeta ? {
          pixabayMusicId: bgmTrackMeta.id,
          title: bgmTrackMeta.title ?? null,
          pageURL: bgmTrackMeta.pageURL ?? null,
          sourceDurSec: bgmTrackMeta.duration ?? null,
    ...
    return {
      ...
      insertions: {
        ...
        clientLibrarySize: clientLibraryCount,             // count BEFORE stock merge
