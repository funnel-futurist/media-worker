  ...
  try {
    ...
    if (!opts.skipBroll) {
      ...
      if (opts.pixabayEnabled && !opts.skipPixabay) {
        // PR-F: shouldFetchStock now defaults to ai_blend mode — always
        // triggers when pixabayEnabled so the picker sees both pools.
        // BROLL_STOCK_MODE=gap_only is an escape hatch for the old behavior.
        stockMode = String(process.env.BROLL_STOCK_MODE ?? 'ai_blend');
        const blendRatio = Number(process.env.BROLL_STOCK_BLEND_RATIO ?? 0.4);
      ...
      if (library.length > 0) {
        ...
        const maxStockRatio = Number(process.env.BROLL_MAX_STOCK_RATIO ?? 0.55);
        const targetStockRatio = Number(process.env.BROLL_STOCK_BLEND_RATIO ?? 0.4);
