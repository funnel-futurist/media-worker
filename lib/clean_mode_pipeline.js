...
export function buildPipelineOpts(req) {
  return {
    ...
    // 2026-06-09: bypass slate_detect entirely. Useful when the detector
    // keeps false-positiving on the actual hook (EnableSNP Saturday Jun 13:
    // title "A Calm Reminder About June 30" + hook "A quiet reminder for
    // any family…" shared the "reminder" token, so the LLM kept eating the
    // hook). When true, slateMetadata stays null and no slate cut is
    // applied. Operator is expected to pre-trim the spoken intro out of
    // the raw before firing — otherwise the intro will leak through.
    ...
    // Phase 1B: standalone opt-in to run segment detection + logging even when
    // cleanup is OFF (telemetry on how often multi-part uploads happen).
    // Detection ALSO runs automatically whenever rawVideoCleanup is on, to feed
    // the boundary-aware segmentHint into cleanup. Default OFF.
...
export async function runCleanModePipeline(req) {
  ...
  try {
    ...
    let slateError = null;
    // 2026-06-09: opt-out flag. When opts.skipSlate is true, bypass slate
    // detection entirely — useful for edge cases where the title and the
    // actual hook share key tokens (EnableSNP Saturday Jun 13: title is
    // "A Calm Reminder About June 30", hook is "A quiet reminder for any
    // family…" — the LLM kept judging the hook as more slate and ate it).
    // When the operator has pre-trimmed the spoken intro from the raw
    // already, slate_detect has nothing useful to do; skipping it avoids
    // false-positive cuts into actual content.
    ...
    // so cleanup never cuts ACROSS a detected reset. Runs when cleanup is on
    // (to feed the hint) OR when segmentDetect is explicitly set (telemetry).
    // Non-fatal: on error we treat the upload as a single clip.
