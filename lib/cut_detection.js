...
export function detectDeterministicCuts(words, options = {}) {
  ...
  //
  // PR-AD: optionally classify FIRST so safe cuts (sentence boundaries,
  // post-sentence dead air, long post-comma dead air) bypass the cap.
  // The cap exists to prevent runaway over-cutting on misclassified
  // content; safe cuts have already been individually safety-vetted by
  // annotateSafety, so capping them is what caused 4-second silences to
  // get DROPPED on jobId 7082844a. With protectSafeCutsFromCap=true the
  // cap only counts non-safe cuts against the budget.
  //
  // protectSafeCutsFromCap defaults false to preserve the original
  // creative-engine parity tests' expected behaviour. The orchestrator
  // opts in via clean_mode_pipeline.js.
  const protectSafeCutsFromCap = options.protectSafeCutsFromCap ?? false;
  let postCap;
  let classified;
  if (protectSafeCutsFromCap && sourceDuration != null && sourceDuration > 0) {
    // Classify, then cap only the RISKY cuts. Safe AND soft both pass
    // through unconditionally so callers using `safe_and_soft` mode
    // don't lose mid-priority cuts to the cap either — only the truly
    // risky ones (where classification couldn't confidently say it's
    // OK to cut) compete for the cap budget. Risky cuts compete for
    // `maxTotalSec` *independently* of how much time the protected
    // cuts consume: each protected cut was individually safety-vetted
    // so it shouldn't indirectly evict risky cuts via shared bookkeeping.
    const classifiedAll = splitMerged.map((c) => annotateSafety(c, words, options));
    const maxTotalSec = sourceDuration * maxCutFraction;
    const protectedOnes = classifiedAll.filter((c) => c.safety === 'safe' || c.safety === 'soft');
    const riskyOnes = classifiedAll.filter((c) => c.safety === 'risky');
    let riskyBudget = maxTotalSec;
    const cappedRisky = [];
    for (const c of riskyOnes) {
      const dur = c.end - c.start;
      if (dur > riskyBudget) {
        // eslint-disable-next-line no-console
        console.warn(
          `[deterministic_cuts] risky cap reached at ${maxTotalSec.toFixed(2)}s — dropping risky cut ` +
          `[${c.start.toFixed(2)}, ${c.end.toFixed(2)}] (${c.safetyReason}) — safe + soft cuts protected`,
        );
        continue;
      }
      cappedRisky.push(c);
    ...
    classified = [...protectedOnes, ...cappedRisky].sort((a, b) => a.start - b.start);
    postCap = classified; // already classified — nothing left to do below
  } else if (sourceDuration != null && sourceDuration > 0) {
    const maxTotalSec = sourceDuration * maxCutFraction;
    const capped = [];
    let runningTotal = 0;
    ...
    postCap = capped;
    classified = postCap.map((c) => annotateSafety(c, words, options));
  } else {
    postCap = splitMerged;
    classified = postCap.map((c) => annotateSafety(c, words, options));
  }
...
export function detectAndClassifyCuts(words, options = {}) {
  ...
  // PR-AD: compute capDropped by running detection again with the cap
  // effectively disabled (maxCutFraction=1.0 → 100% of source) and
  // diffing against allClassified. Spans present in the uncapped result
  // but absent from allClassified were silently dropped by the cap —
  // the audit needs this to distinguish "max_cut_fraction_cap" from
  // "subthreshold" in the diagnostic. Disabled by default to keep this
  // a no-op for non-orchestrator callers (e.g. parity tests).
  let capDropped = [];
  if (options.returnCapDropped) {
    const uncapped = detectDeterministicCuts(words, {
      ...options,
      cutSafetyMode: 'all',
      maxCutFraction: 1.0,
    });
    const keepKey = (c) => `${c.start.toFixed(3)}|${c.end.toFixed(3)}`;
    const keptKeys = new Set(allClassified.map(keepKey));
    capDropped = uncapped.filter((c) => !keptKeys.has(keepKey(c)));
  }

  return { applied, skipped, all: allClassified, capDropped };
}
...
function annotateSafety(cut, words, options = {}) {
  ...
  if (prevEndsInPhrase && CONTINUATION_START_WORDS.has(nextNorm)) {
    // PR-AD: a short comma → conjunction beat is natural rhythm, but a
    // 3-4s pause before "and"/"but"/"that" is dead air every time.
    // Promote to safe once the gap clears the long-comma threshold.
    const cutDurC = cut.end - cut.start;
    const longCommaThresh = options.longCommaPauseAsSafeThreshSec ?? Infinity;
    if (cut.category === 'silence' && cutDurC >= longCommaThresh) {
      return {
        ...cut,
        safety: 'safe',
        safetyReason: `long_comma_then_continuation_${cutDurC.toFixed(2)}s: '${nextNorm}'`,
        contextBefore,
        contextAfter,
      };
    }
  ...
  if (prevEndsInPhrase) {
    // PR-AD: same dead-air promotion as the conjunction branch above —
    // post-comma pauses ≥ longCommaPauseAsSafeThreshSec are no longer
    // natural phrasing, they're awkward silence. Sub-threshold short
    // ones stay 'soft' so they only cut in 'safe_and_soft' mode.
    const cutDurP = cut.end - cut.start;
    const longCommaThreshP = options.longCommaPauseAsSafeThreshSec ?? Infinity;
    if (cut.category === 'silence' && cutDurP >= longCommaThreshP) {
      return {
        ...cut,
        safety: 'safe',
        safetyReason: `long_phrase_boundary_comma_${cutDurP.toFixed(2)}s`,
        contextBefore,
        contextAfter,
      };
    }
