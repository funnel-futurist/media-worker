 ...
 */
const BASE_DEEPGRAM_QUERY_PARAMS = {
  model: 'nova-3',
  smart_format: 'true',
  punctuate: 'true',
  diarize: 'false',
...
const DEEPGRAM_QUERY = new URLSearchParams(BASE_DEEPGRAM_QUERY_PARAMS).toString();

/**
 * PR-AF: build the Deepgram query string including optional keyword
 * boosts. Nova-3 uses the `keyterm` parameter (NOT the older `keywords`
 * parameter which returns 400 on Nova-3). Syntax is
 * `keyterm=<term>:<intensifier>` where intensifier is a number (default
 * appended as :5). For multi-word terms the URLSearchParams encoder
 * handles the space-encoding correctly. We accept either plain terms
 * ("special needs" → defaults to intensifier 5) or already-formatted
 * "<term>:<n>" strings.
 *
 * Hotfix: jobId 7c0bf512 failed with "Keywords are not supported for
 * Nova-3. Please use `keyterm` instead." — renamed the param.
 *
 * @param {string[]} [keywords] e.g. ["special needs", "wondered:7"]
 * @returns {string} query string (no leading `?`)
 */
export function buildDeepgramQuery(keywords) {
  if (!Array.isArray(keywords) || keywords.length === 0) {
    return DEEPGRAM_QUERY;
  }
  const params = new URLSearchParams(BASE_DEEPGRAM_QUERY_PARAMS);
  for (const raw of keywords) {
    if (typeof raw !== 'string') continue;
    const trimmed = raw.trim();
    if (!trimmed) continue;
    // If caller already encoded an intensifier, leave it alone; otherwise
    // append a default :5 intensifier (strong but not silencing the LM).
    const formatted = /:\d+(?:\.\d+)?$/.test(trimmed) ? trimmed : `${trimmed}:5`;
    params.append('keyterm', formatted);
  }
 ...
 */
export async function callDeepgramWithRetry(apiKey, filePath, options = {}) {
  let lastErr;
  // PR-AF: per-job keyword boosts (e.g. "special needs", "wondered") get
  // appended as `keywords=<term>:5` so Deepgram up-weights them when the
  // phonetic match is ambiguous against more common LM candidates like
  // "specialist" or "wandered".
  const query = buildDeepgramQuery(options.keywords);
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      const res = await axios.post(`${DEEPGRAM_URL}?${query}`, stream, {
        headers: {
