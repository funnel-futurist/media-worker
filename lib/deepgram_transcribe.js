 ...
 * boosts. Deepgram's syntax is `keywords=<term>:<intensifier>` where
 * intensifier is a number 1-10 (default 1). For multi-word terms the
 * URLSearchParams encoder handles the space-encoding correctly. We
 * accept either plain terms ("special needs" → defaults to intensifier
 * 5 which is a strong boost without overriding the LM entirely) or
 * already-formatted "<term>:<n>" strings.
 *
...
export function buildDeepgramQuery(keywords) {
  ...
  for (const raw of keywords) {
    ...
    params.append('keywords', formatted);
  }
