 ...
 *   - We want face at output center → crop_y + cropHeight/2 = face_y_in_scaled
 *   - crop_y = offsetY * ih - cropHeight/2
 ...
 * (the fallback) yields a center crop, which is still strictly better than the
 * old hardcoded top crop that clipped low-framed faces.
 *
...
export function buildCropYExpression(offsetY, cropHeight = 1920) {
  ...
  const f = safe.toFixed(4);
  const half = Math.round(cropHeight / 2);
  return `max(0\\,min(ih-${cropHeight}\\,${f}*ih-${half}))`;
}
