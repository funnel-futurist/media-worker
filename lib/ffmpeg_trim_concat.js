 ...
 * were getting killed at the cut step. 20 segments ≈ 40 trim chains
 * (video + audio) which is comfortable for Railway's memory budget.
 */
 ...
 * single-stream pass.
 */
async function _batchedTrimConcat(inputPath, outputPath, keepSegments, applyLoudnorm, encoderArgs, timeoutMs) {
  ...
  const batches = [];
  for (let i = 0; i < keepSegments.length; i += BATCH_THRESHOLD) {
    batches.push(keepSegments.slice(i, i + BATCH_THRESHOLD));
  }
  ...
  // Pass 2: concat all batch files with the concat demuxer (low memory).
  const concatTarget = applyLoudnorm ? join(batchDir, 'concat_raw.mp4') : outputPath;
  ...
  const concatCmd =
    `ffmpeg -y -f concat -safe 0 -i "${concatListPath}" -c copy "${concatTarget}"`;
  await execAsync(concatCmd, { timeout: timeoutMs, maxBuffer: 50 * 1024 * 1024 });

  // Pass 3: apply loudnorm if needed (single-stream, low memory).
  if (applyLoudnorm) {
    const loudnormCmd =
      `ffmpeg -y -i "${concatTarget}" ` +
      `-af "loudnorm=I=-16:TP=-1.5:LRA=11" ` +
      `-c:v copy -c:a aac -b:a 192k ` +
      `"${outputPath}"`;
    await execAsync(loudnormCmd, { timeout: timeoutMs, maxBuffer: 50 * 1024 * 1024 });
  }
}
