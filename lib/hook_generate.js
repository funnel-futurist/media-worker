...
/**
 * PR-V (2026-05-13): defensively extract the first balanced JSON object
 * substring from a string. Used as a fallback when direct JSON.parse()
 * fails because Gemini wrapped the JSON in prose ("Here is the JSON
 * request: {...}"), markdown fences (```json\n{...}\n```), or trailing
 * commentary ("{...} — hope this helps!").
 *
 * Real-world failure that motivated this: jobId af9bdd6c on
 * 2026-05-13 — Gemini 2.5 Flash returned "Here is the JSON request"
 * before the actual JSON, even though responseMimeType:'application/json'
 * was set. This helper recovers the valid object hidden inside.
 *
 * Brace-balance aware (handles strings + escapes) so nested objects
 * inside the hook payload don't break the scan. Returns null if no
 * balanced object can be found.
 *
 * Applied to BOTH Pro and Flash responses in _tryOneModel as defense
 * in depth — Pro typically respects JSON mode, but defensive parsing
 * costs us nothing and protects against future model drift.
 *
 * @param {string} text
...
/**
 * 2026-06-10: post-process a validated hook text to uppercase known brand
 * terms (e.g. "Enable" → "ENABLE" for the ENABLE / EnableSNP brand). The
 * Gemini prompt enforces "Title Case" which strips brand stylization,
 * so without this, hooks ship with "Enable" even though the brand renders
 * all-caps everywhere else (caption, doc title, internal copy).
 *
 * Whole-word, case-insensitive replace per term. Word boundaries protect
 * adjacent letters — `\bEnable\b` matches "Enable" but NOT "enabled" or
 * "Enables". Punctuation around the term doesn't break the match (regex
 * boundary `\b` sits between word and non-word characters).
 *
 * Safe on empty / undefined / non-array input.
 *
 * @param {string} text
 * @param {string[]|undefined} terms  e.g. ['ENABLE']
 * @returns {string}
 */
export function applyBrandAllCaps(text, terms) {
  ...
  for (const term of terms) {
    ...
    out = out.replace(new RegExp(`\\b${escaped}\\b`, 'gi'), upper);
  }
  return out;
}
...
async function _tryOneModel({
  ...
  // PR-V: defensive JSON extraction. Direct JSON.parse first (the
  // happy path — happens 99% of the time when responseMimeType is
  // honored). If that fails, try to extract the first balanced JSON
  // object substring from the text and parse THAT — this rescues
  // responses where the model added prose preamble ("Here is the
  // JSON request: {...}"), markdown fences (```json\n{...}\n```),
  // or trailing commentary ("{...} — hope this helps!"). Applied
  // to BOTH Pro and Flash responses as defense in depth.
  //
  // Only "truly malformed" responses (no balanced {} anywhere, or
  // syntactic JSON errors even after extraction) keep the original
  // invalid_json behavior. The raw output is logged so we can
  // diagnose drift.
  let parsed;
  try {
    ...
    if (extracted === null) {
      return {
        ok: false,
        reason: 'invalid_json',
        detail: `parse failed: ${directErr instanceof Error ? directErr.message : String(directErr)} (no extractable {...}) — raw: ${textOut.slice(0, 120)}`,
      };
    }
    try {
      ...
      return {
        ok: false,
        reason: 'invalid_json',
        detail: `parse failed direct + extracted: direct=${directErr instanceof Error ? directErr.message : String(directErr)} | extracted=${extractedErr instanceof Error ? extractedErr.message : String(extractedErr)} — raw: ${textOut.slice(0, 120)}`,
      };
    }
  }
...
/**
 * 2026-06-10: derive a hook from a content title when LLM generation fails.
 * Uses the same title-extraction logic as the portal's auto-slateHint flow
 * (split on common separators, take the trailing meaningful segment, swap
 * underscores → spaces, trim). Then trims to `introMaxWords` so we don't
 * exceed the 8-word overlay budget.
 *
 * Returns null when the title doesn't yield a usable phrase (empty / too
 * short after extraction). Caller treats null as "no fallback, skip hook
 * overlay" — same behavior as before this fallback existed.
 *
 * Internal helper for generateHookText; exported for unit tests.
 *
 * @param {string|undefined} title
 * @param {number} introMaxWords
 * @returns {string|null}
 */
export function deriveHookFromTitle(title, introMaxWords = DEFAULT_INTRO_MAX_WORDS) {
  ...
  const base = (cleaned.length > 0 ? cleaned : raw).replace(/[^A-Za-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
  if (base.length === 0) return null;
  // Title Case (so the deterministic fallback matches the LLM contract's
  // wording style). Brand normalization is applied SEPARATELY by the
  // caller AFTER this so explicit all-caps brand terms win over Title Case.
  const titleCased = base
    .split(' ')
    .map((w) => w.length === 0 ? w : w[0].toUpperCase() + w.slice(1).toLowerCase())
    .join(' ');
  const words = titleCased.split(/\s+/).slice(0, introMaxWords).join(' ');
...
export async function generateHookText({
  ...
  // 2026-06-10: optional brand-stylization preservation. Whitelist of terms
  // (e.g. ['ENABLE']) to force ALL-CAPS in the generated hook text. Applied
  // AFTER the LLM (which would otherwise Title-Case "enable" → "Enable" per
  // the prompt's wording rule). Same normalization is also applied to the
  // titleForHookFallback path below.
  brandTermsAllCaps,
  // 2026-06-10: deterministic fallback for when Pro AND Flash both fail
  // validation/parsing/etc. Chelsea/Phil EnableSNP Wed Jun 10 + Fri Jun 12
  // shipped without an overlay because the LLM kept generating hooks that
  // failed validateHookText (probably end-punctuation or non-ascii). Skip-
  // and-warn used to be the floor; now when this is provided we derive a
  // hook from it via deriveHookFromTitle → never silently skip the overlay.
  titleForHookFallback,
}) {
  ...
  if (proResult.ok) {
    return { ...proResult, hookText: applyBrandAllCaps(proResult.hookText, brandTermsAllCaps) };
  }
  ...
  if (enableFlashFallback && isRetryableOnFlash(proResult.reason)) {
    ...
    if (flashResult.ok) {
      ...
      return { ...flashResult, hookText: applyBrandAllCaps(flashResult.hookText, brandTermsAllCaps) };
    }
    console.log(
      `[hook_generate] Flash fallback ALSO failed (${flashResult.reason}); checking title fallback`,
    );
    proResult.triedFlash = true;
    proResult.detail = `${proResult.detail} | flash_fallback_${flashResult.reason}: ${(flashResult.detail ?? '').slice(0, 120)}`;
  }

  // 3. Last-resort deterministic title fallback. Never ship a reel without
  //    an overlay when we have a usable title to derive from.
  if (typeof titleForHookFallback === 'string' && titleForHookFallback.trim().length > 0) {
    const derived = deriveHookFromTitle(titleForHookFallback, introMaxWords);
    if (derived) {
      const finalHook = applyBrandAllCaps(derived, brandTermsAllCaps);
      console.log(
        `[hook_generate] LLM hook failed (${proResult.reason}); using deterministic title-fallback hook: "${finalHook}"`,
      );
      return { ok: true, hookText: finalHook, model: 'fallback_title', usedTitleFallback: true };
    }
  }

  // 4. No fallback available — skip-and-warn (legacy behavior).
  console.log(
    `[hook_generate] LLM hook failed (${proResult.reason}) and no title fallback available; skip-and-warn`,
  );
  return proResult;
}
...
async function defaultCallGemini({ model, apiKey, systemPrompt, userPrompt }) {
  ...
  const body = {
    ...
    generationConfig: {
      ...
      temperature: 0.7,
      // PR-W (2026-05-14): bumped 256 → 2048 after 4 consecutive hook-gen
      // failures (jobIds af9bdd6c, 9cfb13d8, 7fb18534, fd45c7c7) all
      // showed token-budget truncation symptoms:
      //   Pro: `{` (1 char), `{"` (2 chars), `Here is the JSON requested:`
      //        (~30 chars, no JSON object at all)
      //   Flash: no_candidate (empty response — candidates[0].content.parts
      //          was missing, suggesting Flash burned the entire budget on
      //          internal thinking and emitted nothing)
      // Hook output itself is tiny (~10 tokens for `{"hookText": "8 words"}`)
      // but Gemini 3.1 Pro Preview uses internal "thinking" tokens that
      // count against maxOutputTokens. 256 wasn't enough for thinking +
      // emit. Same root cause as PR #128's bump on broll_picker
      // (8192 → 16384). 2048 gives a generous reasoning budget while
      // staying well below Pro's 65536 cap, and we only pay for tokens
      // actually emitted.
