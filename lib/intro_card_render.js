 ...
 *
 * **PR-Z (2026-05-14) — N-line stacking bug + placement fix.** PR-Y
 * shipped with a y-position formula that only enumerated two cases
 * (1 line, 2 lines) — every line past the second got the same y as
 * line 2. The 3-line "Planning With Kids Home Is Harder" render
 * collapsed lines 2 and 3 on top of each other into the visible
 * garbage "Planning With" / "'s Harder". The fix replaces the
 * branching expression with `blockTop + idx * lineHeight`, which is
 * arity-agnostic. Same change also bumps LINE_SPACING_FACTOR 1.15→1.30
 * (the 8px stroke + 6px shadow needed more breathing room) and biases
 * the block's vertical anchor to 0.38×h (upper-middle) so the title
 * sits above the speaker's torso rather than directly over it.
 *
 * **PR-AA (2026-05-14) — second-pass spacing + placement.** PR-Z's
 * 1.30× spacing still left descenders of line N touching ascenders of
 * line N+1 on the "Decisive Parents / About The Future" render — at
 * 96pt the visual line height (font + 8px stroke + 6px shadow) is
 * ~120px, so a 125px centre-to-centre gap reads as crowded. Bumped to
 * 1.50× (~144px gap → ~24px clear sky between lines). The 0.38×h
 * vertical anchor also still landed on the talking-head's face — faces
 * dominate the upper-middle of portrait reels, not dead centre.
 * Anchor moved to 0.20×h (top quintile) so the title sits above the
 * face entirely while staying inside the 120px top safe margin.
 *
...
// Vertical spacing between stacked lines as a multiple of font size. A
// 96pt Montserrat Black glyph is ~96px tall, the 8px stroke adds 16px,
// and the 6px drop shadow adds another 6px on the bottom — the visual
// height per line is closer to ~120px. 1.30× (= 125px gap centre-to-
// centre at 96pt) left descenders of line N visually touching the
// ascenders of line N+1 on the first PR-Z render. 1.50× gives ~26px
// of clear sky between adjacent lines and reads as deliberately spaced.
const LINE_SPACING_FACTOR = 1.50;
// Vertical anchor for the title block. The previous 0.38×h target
// still landed squarely on the speaker's face in a typical portrait
// talking-head shot — faces dominate the upper-middle, not the dead
// centre. 0.20×h places the title block in the top quintile so it sits
// above the face entirely (clear of the eyes/forehead) while staying
// well below the 120px top safe margin.
const VERTICAL_CENTER_RATIO = 0.20;
...
export function buildIntroOverlayArgs({
  ...
  // Compute the title block's top-edge y once. The block is
  // `lines.length * lineHeightPx` tall; we anchor its centre at
  // VERTICAL_CENTER_RATIO × height so the title sits upper-middle
  // instead of dead-centre over the speaker's torso. Width/height are
  // fixed at filter build time (1080×1920 by default) so we pre-compute
  // numeric pixel offsets — far simpler than ffmpeg expressions and
  // generalises to N lines without per-arity branching.
  const blockHeightPx = lines.length * lineHeightPx;
  const blockTopPx = Math.round(height * VERTICAL_CENTER_RATIO - blockHeightPx / 2);
  ...
  const drawtextFilters = lines.map((line, idx) => {
    ...
    // Each line stacks at blockTop + (idx * lineHeight). The previous
    // implementation hardcoded only two y positions, so a 3-line wrap
    // rendered lines 2 and 3 at the same y, producing the "Planning With"
    // / "'s Harder" collision Shannon flagged on the first PR-Y render.
    const yPx = blockTopPx + idx * lineHeightPx;
