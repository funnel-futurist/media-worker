...
export async function probeStreams(filePath) {
  ...
  // PR #193: parse r_frame_rate ("30/1" or "30000/1001") into a numeric fps
  // so pre-flight validation can reject clips outside the 12-60 fps range
  // that libx264's filter chain reliably handles.
  const parseFps = (rateStr) => {
    if (typeof rateStr !== 'string' || !rateStr.includes('/')) return 0;
    const [num, den] = rateStr.split('/').map((s) => parseInt(s, 10));
