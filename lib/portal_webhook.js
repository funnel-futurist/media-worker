...
export function buildReelEditedPayload({
  ...
  bannerApplied,
  introHookApplied,
  // 2026-06-01: the client-facing aiEditMode preset that actually ran for
  // this job. Optional → portals that don't yet read it just ignore the
  // extra field; portals that DO can store it on content_items.ai_edit_mode
  // so subsequent edits/re-fires default to the same choice.
  aiEditMode,
}) {
  ...
  if (typeof aiEditMode === 'string' && aiEditMode.length > 0) payload.aiEditMode = aiEditMode;
  return payload;
}
