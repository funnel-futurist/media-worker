...
export function buildEditNotesSummary(pipelineResult) {
  ...
  // Phase 2 (best-take + end-on-CTA): surface the LOG-ONLY analysis on the
  // Deliverables card so reviewers can see CTA presence + proposed best-take
  // drops without digging through Railway logs. Summary only — NO cuts are
  // applied from this (best-take is log-only; endOnCta gates the trim
  // separately). Only present when the analysis ran (bestTakeDetect/endOnCta on)
  // and succeeded; on error we omit rather than mislead with "No CTA detected".
  const bt = r.steps?.bestTakeCta;
  if (bt && !bt.error) {
    const ctaPart = bt.cta?.found
      ? `CTA detected @${Number(bt.cta.end_time).toFixed(1)}s (${Number(bt.cta.confidence).toFixed(2)})${bt.trimApplied ? ', trimmed' : ''}.`
      : 'No CTA detected.';
    const n = bt.proposedDropCount ?? 0;
    const btPart = n > 0 ? `Best-take: ${n} proposed drop(s).` : 'Best-take: no proposed drops.';
    parts.push(`${ctaPart} ${btPart}`);
  }
...
export function buildReelEditedPayload({
  ...
  preCaptionVideoUrl,
  subtitleAssUrl,
  // Ad pipeline signals used to compute edit_intensity portal-side.
  insertionsCount,
  bannerApplied,
  introHookApplied,
}) {
  const payload = { contentItemId, clientId, editedUrl, outcome: 'success' };
  ...
  // Ad-routing taxonomy signals — portal classifies edit_intensity from these.
  // Always include when defined so the portal can store the taxonomy line
  // even for reel jobs (cheap, additive). Portal only uses them for ads.
  if (typeof insertionsCount === 'number') payload.insertionsCount = insertionsCount;
  if (typeof bannerApplied === 'boolean') payload.bannerApplied = bannerApplied;
  if (typeof introHookApplied === 'boolean') payload.introHookApplied = introHookApplied;
  return payload;
}

/**
 * PR-181b: build the FAILURE payload for /api/editor/callback/reel.
 *
 * When the worker fails (orchestrator throw, result.error partial-data,
 * signed URL mint failure, etc.), POST this shape so the portal can
 * IMMEDIATELY flip the row to pipeline_status='edit_failed' with a
 * specific error message — instead of waiting 15 min for the stuck-row
 * cron to mark it 'worker_timeout'.
 *
 * The portal endpoint reads `outcome` to branch between success
 * (existing edit_file_url update + Slack handoff) and failure (this
 * shape: row update + diagnostic Slack ping).
 *
 * @param {object} args
 * @param {string} args.contentItemId
 * @param {string} args.clientId
 * @param {string} args.jobId
 * @param {string} args.failedStep        e.g. 'compose', 'subtitleBurn', 'orchestrator', 'signStorageUrl'
 * @param {string} args.errorMessage      short human-readable error (max 1000 chars)
 * @param {Array<string>} [args.warnings] any warnings already collected
 * @param {object} [args.stepTimings]     partial result.steps if available
 * @returns {object} payload
 */
export function buildReelFailedPayload({
  contentItemId,
  clientId,
  jobId,
  failedStep,
  ...
  if (Array.isArray(warnings) && warnings.length > 0) {
    payload.warnings = warnings.slice(0, 50).map((w) => String(w).slice(0, 400));
  }
  if (stepTimings && typeof stepTimings === 'object') {
    payload.stepTimings = stepTimings;
  }
  // PR (no-speech terminal): only emit `terminal` when the worker is sure
  // a retry can't help (e.g. no speech in the source). Omitted otherwise so
  // existing retryable-failure payloads are byte-identical. The portal's
  // retry-failed-edits sweep is the companion change that reads this flag
  // to skip non-retryable rows instead of burning MAX_RETRIES.
  if (terminal === true) {
...
export async function postReelEditedCallback(args) {
  ...
  // PR-181b: validator branches on outcome. Success payloads require
  // editedUrl; failure payloads require errorMessage. Both require the
  // contentItemId + clientId pair so the portal can locate the row.
  if (!payload.contentItemId || !payload.clientId) {
    return {
      ok: false,
      attempts: 0,
      error: 'payload missing required fields (contentItemId, clientId)',
    };
  }
  const isFailure = payload.outcome === 'failed';
  if (!isFailure && !payload.editedUrl) {
    return {
      ok: false,
      attempts: 0,
      error: 'success payload missing required field (editedUrl)',
    };
  }
  if (isFailure && !payload.errorMessage) {
    return {
      ok: false,
      attempts: 0,
      error: 'failure payload missing required field (errorMessage)',
    };
  }
