 ...
 *                                             cap silently evicted. When
 *                                             provided, audit rows whose
 *                                             span overlaps a capDropped cut
 *                                             surface `reason: 'max_cut_fraction_cap'`
 *                                             instead of the catch-all
 *                                             `'subthreshold'`.
 * @returns {SilenceAudit}
 */
export function auditSilenceCoverage({ mergedSilences, applied, skipped, capDropped = [] }) {
  const rows = [];
  ...
  for (const span of mergedSilences) {
    ...
    // Both surface as 'dropped' decisions, but the reason slug tells the
    // operator whether to raise the cap or look at the per-span thresholds.
