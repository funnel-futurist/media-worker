...
// PR-AO: stopwords stripped from slateHint fuzzy match. Keeps only content
// words so a hint like "Imagine June Without This Lingering" matches Phil's
// audio even with minor punctuation/case differences. Intentionally small —
// we keep nouns/verbs/adjectives/adverbs intact for high-specificity matching.
const SLATE_HINT_STOPWORDS = new Set([
  'a', 'an', 'the', 'to', 'of', 'for', 'in', 'on', 'at', 'with', 'by', 'and',
  'or', 'but', 'is', 'are', 'was', 'were', 'be', 'been', 'being',
]);

function tokenizeForHint(text) {
  if (typeof text !== 'string') return [];
  return text
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, ' ')
    .split(/\s+/)
    .filter((w) => w.length > 0 && !SLATE_HINT_STOPWORDS.has(w));
}

/**
 * PR-AO: fuzzy-match a transcript sentence against a slateHint. Returns true
 * when the sentence contains at least `minMatchRatio` of the hint's content
 * tokens. Punctuation and case are normalized; stopwords are ignored.
 *
 * Example: hint "Imagine June Without This Lingering" → content tokens
 * [imagine, june, without, this, lingering]. Sentence "Imagine June without
 * this lingering." → matches 5/5 = 1.0 ≥ 0.6 → true.
 *
 * @param {string} sentence
 * @param {string} hint
 * @param {number} [minMatchRatio=0.6]
 * @returns {boolean}
 */
export function fuzzyMatchesSlateHint(sentence, hint, minMatchRatio = 0.6) {
  const hintTokens = tokenizeForHint(hint);
  if (hintTokens.length === 0) return false;
  const sentenceTokenSet = new Set(tokenizeForHint(sentence));
  if (sentenceTokenSet.size === 0) return false;
  let matched = 0;
  for (const t of hintTokens) {
    if (sentenceTokenSet.has(t)) matched += 1;
  }
  return matched / hintTokens.length >= minMatchRatio;
}

/**
 * 2026-06-09: when a slateHint matched a sentence (via `fuzzyMatchesSlateHint`),
 * find the precise end_ms of the LAST hint token within the sentence's words.
 * The previous behavior — cutting through the sentence's `endMs` — over-cut
 * whenever Deepgram fused the spoken slate AND the hook into one big sentence
 * with no period between them (Chelsea/Phil Saturday Jun 13 reel: hint
 * "saturday june 13th the calm reminder about june 30" matched the giant
 * fused sentence, then the cut ran all the way through "...thinking about
 * enable", taking the hook with it).
 *
 * Walks the sentence's words left-to-right, matching hint tokens in order.
 * Returns the end_ms of the last sentence-word that aligned with a hint token.
 * Requires the same 0.6 match ratio fuzzyMatchesSlateHint uses, so a sentence
 * that already passed fuzzyMatchesSlateHint will always yield a precise end
 * here (assuming reasonable input).
 *
 * @param {Array<{word: string, end_ms: number}>} sentenceWords
 * @param {string} hint
 * @param {number} [minMatchRatio=0.6]
 * @returns {number|null}  end_ms in source-time milliseconds, or null when
 *   match doesn't meet the ratio (caller falls back to sentence end).
 */
export function findHintEndMsInSentence(sentenceWords, hint, minMatchRatio = 0.6) {
  if (!Array.isArray(sentenceWords) || sentenceWords.length === 0) return null;
  const hintTokens = tokenizeForHint(hint);
  if (hintTokens.length === 0) return null;

  // Pre-tokenize each sentence word once (skip stopwords + punctuation just
  // like tokenizeForHint does for the hint). We keep the original index so
  // we can recover end_ms after token alignment.
  const sentenceTokens = sentenceWords
    .map((w, i) => {
      const tokens = tokenizeForHint(w?.word ?? '');
      // A single transcript word usually yields 0 or 1 token. If a word
      // somehow tokenizes to multiple (e.g. punctuation embedded), we
      // associate them all with the same source index.
      return tokens.map((t) => ({ token: t, idx: i }));
    })
    .flat();
  if (sentenceTokens.length === 0) return null;

  // In-order alignment: walk sentence tokens, advance hint pointer when the
  // current sentence token equals the next-expected hint token. Track the
  // sentence index of the most recent successful alignment.
  let hintPtr = 0;
  let lastMatchedSentenceIdx = -1;
  for (const { token, idx } of sentenceTokens) {
    if (hintPtr >= hintTokens.length) break;
    if (token === hintTokens[hintPtr]) {
      lastMatchedSentenceIdx = idx;
      hintPtr += 1;
    }
  }

  if (hintPtr / hintTokens.length < minMatchRatio) return null;
  if (lastMatchedSentenceIdx < 0) return null;
  const endMs = sentenceWords[lastMatchedSentenceIdx]?.end_ms;
 ...
 *
 * PR-AO (2026-05-25): optional `slateHint` — when provided, fuzzy-match
 * against every sentence in the 20s window. Any matched sentence is treated
 * as a slate sentence even if it appears AFTER the last meta marker (i.e.,
 * past the existing PR-AM look-ahead cap). Used for Phil's new recording
 * style where he drops the trailing "Selected option / Final revised version"
 * markers — only date + title remain, and the title would otherwise survive.
 * The hint must come from a known source (filename / content_items.title);
 * we never assume a title sentence is slate without it.
 *
 * @param {Array<{word: string, start_ms: number, end_ms: number}>} words
 * @param {object} [opts]
 * @param {string} [opts.slateHint]  expected title text; fuzzy-matched against
 *   each sentence in the 20s window. Match ratio threshold: 0.6 of hint's
 *   content tokens must appear in the sentence (stopwords ignored).
 * @returns {{endSec: number, sentences: string[], matchedMarkers?: string[], matchedHint?: string} | null}
 */
export function detectDeterministicSlateFloor(words, opts = {}) {
  if (!Array.isArray(words) || words.length === 0) return null;
  const earlyWords = words.filter((w) => w.end_ms <= 20_000);
  if (earlyWords.length === 0) return null;

  // Split into sentences. We KEEP the word-array per sentence (2026-06-09)
  // so the hint-end finder below can locate the precise cut point within
  // sentences that Deepgram fused (no period between slate and hook).
  const sentences = [];
  let buf = [];
  for (const w of earlyWords) {
    buf.push(w);
  ...
  // PR-AO: scan for slateHint match across the entire 20s window. If the hint
  // matches a sentence that's later than the last meta marker, extend the
  // cut through that sentence. This catches Phil's date+title pattern where
  // the title sits past the date marker with no trailing meta cue.
  let hintIdx = -1;
  let matchedHint = null;
  const hint = typeof opts.slateHint === 'string' ? opts.slateHint.trim() : '';
  if (hint.length > 0) {
    for (let i = 0; i < sentences.length; i++) {
      if (fuzzyMatchesSlateHint(sentences[i].text, hint)) {
        hintIdx = i;
        matchedHint = sentences[i].text;
        break;  // first match wins — slate is at the start
      }
    }
  }

  const lastIdx = Math.max(lastMetaIdx, hintIdx);
  if (lastIdx < 0) return null;

  // Cut everything up through the end of the LAST slate-bearing sentence.
  // This pulls along any non-meta sentences (titles, "what we'll talk
  // about" preambles) that appear BEFORE the last marker/hint.
  //
  // 2026-06-09: when the WINNING boundary is a hint match (not a meta marker),
  // cut at the PRECISE end of the matched hint INSIDE the sentence — not the
  // sentence's end_ms. Why: Deepgram occasionally fuses the spoken slate AND
  // the opening hook into one giant sentence (no period between them). The
  // old "sentence.endMs" behavior then over-cut all the way through the hook
  // (EnableSNP Saturday Jun 13: hint "saturday june 13th the calm reminder
  // about june 30" matched the fused sentence; the cut ran through "...thinking
  // about enable", losing the hook). Falls back to sentence.endMs when the
  // hint-end finder can't locate a precise boundary (shouldn't happen if
  // fuzzyMatchesSlateHint already accepted the sentence, but be safe).
  let endMs;
  if (hintIdx === lastIdx && hintIdx >= 0) {
    const sentence = sentences[hintIdx];
    const precise = findHintEndMsInSentence(sentence.words ?? [], hint);
    endMs = typeof precise === 'number' ? precise : sentence.endMs;
  } else {
    endMs = sentences[lastIdx].endMs;
  }
  const inSlateWindow = sentences.slice(0, lastIdx + 1).map((s) => s.text);
  return {
    endSec: endMs / 1000,
    sentences: inSlateWindow,
 ...
 * @param {string} [opts.slateHint]              PR-AO: expected title text;
 *   fuzzy-matched against transcript sentences to catch Phil's date+title
 *   pattern when the title sits past the last meta marker.
...
export async function detectSlate(input, opts = {}) {
  ...
  if (deterministicFloor) {
    ...
    const hint = deterministicFloor.matchedHint;
    console.log(
      `[slate_detect] deterministic-floor: ${deterministicFloor.endSec.toFixed(2)}s — ` +
        `cut window covers ${deterministicFloor.sentences.length} sentence(s) ` +
        (hint
          ? `(slateHint matched: "${hint.slice(0, 60)}"; ${matched.length} meta marker(s))`
          : `ending at last meta marker (matched ${matched.length}): ` +
            matched.map((s) => `"${s.slice(0, 40)}"`).join(', ')),
    );
  }

  // 2026-06-09 follow-up: when the deterministic floor matched a user-
  // provided `slateHint`, it has located the END of the slate precisely
  // via in-sentence token alignment. That's HIGH-CONFIDENCE — the operator
  // told us where the slate ends, and the transcript confirmed it. In that
  // case, trust the hint-end as AUTHORITATIVE: do not let Gemini extend
  // past it. The previous "take the larger" rule (lines below) was the
  // root cause of the EnableSNP Saturday Jun 13 over-cut even AFTER
  // PR #222 fixed the floor to return precise hint-end — Gemini was
  // independently judging the entire opening as slate (8+ seconds, eating
  // the hook), and that larger value was winning.
  //
  // Important: this clamp only kicks in when matchedHint is set (the
  // operator passed a hint AND it actually matched the transcript).
  // Meta-marker-only path (no hint) still uses the old "larger wins" rule,
  // which protects Phil's original "Selected option / Final version"
  // workflow where Gemini sometimes under-cuts.
  if (deterministicFloor?.matchedHint) {
    if (
      parsed?.isSlate &&
      typeof parsed.slateEndSeconds === 'number' &&
      parsed.slateEndSeconds > deterministicFloor.endSec
    ) {
      console.log(
        `[slate_detect] hint-end is authoritative: clamping gemini=${parsed.slateEndSeconds.toFixed(2)}s → ${deterministicFloor.endSec.toFixed(2)}s (matched hint: "${deterministicFloor.matchedHint.slice(0, 60)}")`,
      );
    }
    parsed = {
      isSlate: true,
      ...
      identifier: 'deterministic_floor_hint',
    };
  } else if ((!parsed.isSlate || parsed.slateEndSeconds == null || parsed.slateEndSeconds <= 0)) {
