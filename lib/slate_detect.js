...
export async function detectSlate(input, opts = {}) {
  ...
  // PR-AJ (2026-05-19): hook-validator DISABLED. The validator was meant
  // to preserve content hooks that Gemini bundled into the slate, but
  // for Phil's reels (and likely most other clients with a structured
  // slate convention) the "content-looking" sentences are actually
  // TITLE READOUTS that should be cut as slate. Specifically, Phil's
  // convention is: [date] → [title sentence] → [option/version marker]
  // → [actual script]. The title sentence is always ≥4 non-meta words,
  // so it looked like content to the validator. Result: the validator
  // shortened the slate to just before the title, leaving the title +
  // option marker in the reel. Chelsea flagged this on the 2026-05
  // batch: "the intro part needs to be cut."
  //
  // Trusting Gemini's original slateEndSeconds is the safer default.
  // If Gemini over-cuts a real hook in a future case, we'll revisit
  // with a more conservative rule (e.g., requiring zero meta-markers
  // ANYWHERE in the slate window before preserving a content hook).
  // For now: the intro always gets cut, period.
