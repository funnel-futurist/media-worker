...
export function validateSlatePreservesHooks(parsed, words) {
  ...
  // Find the first sentence that's a content hook
...
export async function detectSlate(input, opts = {}) {
  ...
  // Phase PR-AH: post-hoc validator — shorten slate if it swallowed a
  // content hook (rhetorical question, topic opener) between meta markers.
  parsed = validateSlatePreservesHooks(parsed, input.wordTimestamps);

  // The validator may have shortened slateEndSeconds to 0 if the very first
  // sentence was a content hook (no actual slate at all).
  if (!parsed.isSlate || parsed.slateEndSeconds == null || parsed.slateEndSeconds <= 0) {
    return null;
  }
