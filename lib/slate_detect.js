...
const META_MARKER_PATTERNS = [
  ...
  // PR-AK additions: Phil's "Final version." / "Final cut" / "Version N"
  // markers. Caught one of Chelsea's specific complaints: "Phil saying
  // the title AND then 'Final version' before the script."
...
export function detectDeterministicSlateFloor(words) {
  ...
  for (const sentence of sentences) {
    if (isMetaMarker(sentence.text)) {
      ...
      matched.push(sentence.text);
      continue;
    }
    // First non-meta sentence → content begins, stop.
    break;
  }

  if (matched.length === 0) return null;
  return { endSec: endMs / 1000, sentences: matched };
}
  ...
  if (deterministicFloor) {
    console.log(
      `[slate_detect] deterministic-floor: ${deterministicFloor.endSec.toFixed(2)}s — ` +
        `matched ${deterministicFloor.sentences.length} meta sentence(s): ` +
        deterministicFloor.sentences.map((s) => `"${s.slice(0, 40)}"`).join(', '),
    );
  }
