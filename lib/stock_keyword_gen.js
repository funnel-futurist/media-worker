...
transcript. Your job: produce 8 to 10 short search keywords that will
...
KEYWORD DIVERSITY — CRITICAL:
  Each keyword must target a DIFFERENT visual category. The audience will
  see 8-10 b-roll clips in the video — they must not all look the same.
  Spread keywords across these categories:
  - PEOPLE: family, couple, parent-child interactions, group conversations
  - SETTINGS: living room, office, outdoor park, kitchen table
  - ACTIVITIES: walking together, writing, signing, talking, cooking
  - OBJECTS: documents, calendar, clock, phone, notebook
  - CONCRETE METAPHORS (objects/actions only): open door (opportunity),
    hands building together (collaboration), a clock (time). The metaphor
    must be a tangible object or human action — NOT a landscape.

  Every keyword should put PEOPLE, a HUMAN ACTION, or a SPECIFIC OBJECT in
  frame. Stock for these reels must feel warm, specific, and human — generic
  scenery is filler the picker will reject.

  Never generate 2+ keywords that would return visually similar footage
  (e.g., "writing notes notebook" and "book pages literature" are the
  same visual — pick ONE and use the other slot for a different category).

AVOID:
  - Generic landscape / scenery with no people or objects (forest, trees,
    lake, mountains, sunset, sunrise, sky, clouds, generic "nature" or
    "outdoors") — these return pretty-but-irrelevant filler and are the #1
    source of bad picks. Do NOT use them UNLESS the transcript is literally
    about nature or the outdoors. If a beat feels abstract/emotional, anchor
    it to a PERSON or ACTION instead (e.g. "parent waking up morning light",
    not "sunrise"; "family planning at table", not "calm landscape").
  ...
  - Multiple keywords in the same visual category — VARY the visuals
...
function buildUserPrompt({ transcript, clientLibrarySize, coverageGap, durationSec, contentContext }) {
  ...
  const contextLine = contentContext
    ? `CONTENT DOMAIN: ${contentContext}. Keywords MUST be relevant to this domain — reject generic lifestyle footage that doesn't visually support the specific topic.`
    : '';
  return [
    ...
    `Generate 8 to 10 keywords — each targeting a DIFFERENT visual category.`,
    contextLine,
    '',
    `Transcript:`,
    transcriptText,
