...
function adaptStockHitToLibraryRow(hit) {
  ...
  // Tier 2-a: dual-provider stock. Hits carry `provenance` set at fetch
  // time ('pixabay' default for legacy; 'pexels' from lib/pexels_video.js).
  // Defaults to 'pixabay' so older test fixtures that omit provenance
  // continue to behave identically.
  const provenance = hit.provenance === 'pexels' ? 'pexels' : 'pixabay';
  const tagsStr = typeof hit.tags === 'string' ? hit.tags : '';
  const tagList = tagsStr.split(/[,\s]+/).map((t) => t.trim()).filter(Boolean);
  const sourceLabel = provenance === 'pexels' ? 'Pexels' : 'Pixabay';
  const title = tagList.length > 0
    ? `${sourceLabel} stock: ${tagList.slice(0, 3).join(', ')}`
    : `${sourceLabel} stock asset ${hit.id}`;
  ...
  const idPrefix = provenance === 'pexels' ? 'pexels-video' : 'px-video';
  const row = {
    asset_id: `${idPrefix}-${hit.id}`,
    asset_title: title,
    asset_type: 'video',
    ...
    // Tier 2-b: per-asset face-aware crop offset. Default 0.5 = center crop
    // (current behavior pre-Tier-2-b). Pipeline sets this on the hit for
    // landscape stock after detectFaceOffsetX; portrait/square hits skip
    // detection. composeFaceAndBrolls reads `faceCropOffsetX` on each
    // insertion row to build the per-segment cropXExpr.
    faceCropOffsetX: typeof hit.faceCropOffsetX === 'number' ? hit.faceCropOffsetX : 0.5,
    faceCropSource: typeof hit.faceCropSource === 'string' ? hit.faceCropSource : 'default-center',
  };
  if (provenance === 'pexels') {
    row.pexelsVideoId = hit.id;
    row.pexelsPageURL = hit.pageURL ?? null;
    // Attribution recorded in metadata, not burned on-screen.
    row.attributionUser = hit.attributionUser ?? null;
    row.attributionUrl = hit.attributionUrl ?? null;
  } else {
    row.pixabayVideoId = hit.id;
    row.pixabayPageURL = hit.pageURL ?? null;
  }
  return row;
}
...
export function rebalanceClientFirst({
  ...
  // Tier 2-a: 'pexels' joins 'pixabay' as a stock provenance. Anything not
  // 'client' is treated as stock for source-balancing purposes.
  const isStock = (i) => i?.provenance === 'pixabay' || i?.provenance === 'pexels';
