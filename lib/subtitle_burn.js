 ...
 *
 * @param {Array<{ word: string, start_ms: number, end_ms: number }>} words
 * @param {number} maxWords  default 6
 * @param {number} maxDurationSec  default 2.5
 * @returns {Array<{ start: number, end: number, text: string, wordCount: number }>}
 */
export function groupIntoLines(words, maxWords = 6, maxDurationSec = 2.5) {
  const lines = [];
  let buf = [];
...
/**
 * Generate the .ass content using ff_clean_subtitle styling — matches
 * scripts/add_subtitles.ts and lib/hyperframes/render_blueprint.ts:emitCaptions:
 *   - Montserrat 50pt, weight 700 (Bold=-1)
 *   - White primary (&H00FFFFFF), dark outline (&H001A1A1A) 4pt
 *   - Soft drop shadow (Shadow=2)
 *   - PlayRes 1080×1920 (talking-head reel canvas)
 *   - Bottom-center alignment by default, MarginV=200
 *
...
Style: Default,Montserrat,50,&H00FFFFFF,&H000000FF,&H001A1A1A,&H00000000,-1,0,0,0,100,100,1,0,1,4,2,${alignment},40,40,200,1
