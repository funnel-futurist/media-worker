 ...
 *
 * **introOffsetSec (PR-L, 2026-05-13):** when non-zero, every surviving
 * word's start/end gets an ADDITIONAL forward shift of `introOffsetSec`
 * seconds AFTER the cut-merge math. Used when the orchestrator prepends
 * a 5-second AI-generated hook card to the final composition: the cut.mp4
 * timeline is unchanged but the FINAL VIDEO has the speaker starting at
 * t=introOffsetSec, so the subtitle display times need to shift forward
 * to stay aligned. This is purely additive — the PR #127 cut-overlap-merge
 * fix above is unchanged. The intro_timeline_shift regression test
 * exercises both fixes composed.
 *
 ...
 * @param {object} [opts]
 * @param {number} [opts.introOffsetSec=0]  forward shift applied AFTER
 *   the cut math. 0 = legacy behavior (no intro).
 * @returns {Array<{ word: string, start_ms: number, end_ms: number }>}
 */
export function remapWordsThroughCuts(words, appliedCuts, opts = {}) {
  ...
  // 2026-05-11 fix: MERGE overlapping cuts before computing the shift.
  // Previously this function only sorted, which caused the shift sum to
  // overcount overlap regions. Repro: slate [0, 5.76] + silence [0.15, 0.912]
  // + silence [4.45, 5.71] on Phil's Well_Handle_It_This_Summer clip —
  // ffmpeg's buildKeepSegments merged them to a single [0, 5.76] (5.76s
  // removed) but this function summed them as 5.76 + 0.762 + 1.26 = 7.78s,
  // so every word after t=5.76 was placed ~2s too early. Subtitles read
  // 2 seconds ahead of the speaker.
  // Now uses the shared mergeCutSpans helper that buildKeepSegments also
  // uses — both code paths see the identical canonical cut list.
  const cuts = mergeCutSpans(appliedCuts);

  // PR-L: forward shift to absorb the prepended intro card. Computed once
  // here, applied to every surviving word's start/end. Zero by default so
  // pre-PR-L callers behave identically.
  const introOffsetMs = typeof opts.introOffsetSec === 'number' && Number.isFinite(opts.introOffsetSec) && opts.introOffsetSec > 0
    ? Math.round(opts.introOffsetSec * 1000)
    : 0;
  ...
  for (const w of words) {
    ...
    out.push({
      word: w.word,
      start_ms: Math.max(0, w.start_ms - shiftMs) + introOffsetMs,
      end_ms: Math.max(0, w.end_ms - shiftMs) + introOffsetMs,
    });
  }
  return out;
}
 ...
 *
 *   - **Fontname=Montserrat Black** (PR #107) — requests the heaviest face in
 *     the Montserrat family. Falls back via libass/fontconfig to the next
 *     heaviest Montserrat weight if `fonts-montserrat-extra` isn't installed,
 *     and beyond that to the system bold default. PR #107 installs both
 *     `fonts-montserrat` and `fonts-montserrat-extra` in the Dockerfile so the
 *     Black face is always available in production.
 *   - 80pt, weight 700 (Bold=-1) — kept; Bold=-1 is moot when the family
 *     itself is the Black face but provides a sensible fallback weight if
 *     libass resolves to "Montserrat" instead of "Montserrat Black".
 *   - White primary (&H00FFFFFF), dark outline (&H001A1A1A) 4pt
 *   - Soft drop shadow (Shadow=2)
 *   - PlayRes matches the target output (default 1080×1920 reel; 1080×1350 for 4:5 ads)
 *   - Bottom-center alignment + MarginV proportional to playResY
 *     - At 1920: MarginV=520 → baseline at y≈1400 (shoulder/upper-chest)
 *     - At 1350: MarginV≈366 (520 × 1350/1920) → same 27.1% margin-from-bottom ratio
 *
 * @param {Array<{ start, end, text }>} lines
 * @param {Object} [opts]
 * @param {'top'|'bottom'} [opts.placement='bottom']
 * @param {number} [opts.playResX=1080]  ASS canvas width; should match the
 *   ffmpeg output frame width so subtitle positioning is pixel-accurate.
 * @param {number} [opts.playResY=1920]  ASS canvas height; should match the
 *   ffmpeg output frame height.
 * @param {number} [opts.marginV]  Optional explicit MarginV (in ASS units, =px
 *   at the PlayResY scale). When omitted, defaults to `round(520 * playResY/1920)`
 *   so the shoulder/upper-chest caption baseline ratio is preserved at any height.
 * @returns {string}
 */
export function generateAss(lines, opts = {}) {
  const placement = opts.placement === 'top' ? 'top' : 'bottom';
  const alignment = placement === 'top' ? 8 : 2;
  const playResX = Number.isFinite(opts.playResX) && opts.playResX > 0 ? opts.playResX : 1080;
  const playResY = Number.isFinite(opts.playResY) && opts.playResY > 0 ? opts.playResY : 1920;
  // Keep the same 520/1920 ≈ 27.1% margin-from-bottom across output sizes so
  // the caption baseline lands on the speaker's upper chest regardless of
  // whether we render a 9:16 reel (1920 → 520) or a 4:5 ad (1350 → 366).
  const marginV =
    Number.isFinite(opts.marginV) && opts.marginV > 0
      ? Math.round(opts.marginV)
      : Math.round(520 * (playResY / 1920));
...
Style: Default,Montserrat Black,80,&H00FFFFFF,&H000000FF,&H001A1A1A,&H00000000,-1,0,0,0,100,100,1,0,1,4,2,${alignment},40,40,${marginV},1
 ...
 *
 * libass emits fontselect lines in two shapes depending on whether the
 * requested family resolved at all:
 *
 *   1. Verbose miss:
 *      `fontselect: Montserrat not found, ... Using default font`
 *      → caught by the `not found` / `Using default` regex pair below.
 *
 *   2. Arrow notation (the actual format on the Playwright Noble base
 *      image) — emitted whenever fontconfig substitutes ANY family,
 *      including the silent Montserrat → Arial Bold fallback that bit us
 *      on B5:
 *      `fontselect: (Montserrat, 700, 0) -> Arial-BoldMT, 0, Arial-BoldMT`
 *      Pre-PR #108 our regex never caught this format, so the warning
 *      was silently swallowed and the response carried `warnings: []`
 *      while production had a real font fallback in flight. PR #108 adds
 *      arrow-notation detection so future regressions surface.
 *
 * @param {string} stderr
 * @returns {string[]}
 */
export function extractSubtitleWarnings(stderr) {
  ...
  // Arrow-notation fallback (PR #108): match `fontselect: (Family, weight, italic) -> Resolved, ...`
  // and warn whenever the resolved family doesn't include "Montserrat" — which is the actual signal
  // we care about in production. The capture group exposes the resolved family in the warning text
  // so the operator can tell at a glance which fallback face libass picked.
  const arrowMatch = stderr.match(/fontselect:\s*\(([^,)]+),[^)]*\)\s*->\s*([^,\n]+)/i);
  if (arrowMatch) {
    const requested = arrowMatch[1].trim();
    const resolved = arrowMatch[2].trim();
    if (!/montserrat/i.test(resolved)) {
      warnings.push(
        `libass fontselect: requested "${requested}" → resolved "${resolved}" — Montserrat not bound, captions rendered with fallback face`,
      );
    }
  }
  return warnings;
